...
func main() {
	...
	// ══════════════════════════════════════════════════════════════════
	//  خزينةُ المنصة — تُوسَم هنا لا في ترحيل
	// ══════════════════════════════════════════════════════════════════
	//
	// **الوسمُ كان في `0070_treasury_is_a_wallet_not_a_setting.sql`**،
	// **والترحيلُ لا يُعاد.** فمن نظّف القاعدةَ بـ`cleandev` ثمّ زرعها
	// **خرج بقاعدةٍ بلا خزينة** — ولا يظهر ذلك إلّا في `moneycheck`.
	//
	// **وبلا خزينةٍ يُتجاهَل مصروفُ المنصة بصمت**، فتقريرُ الخسائر يبقى
	// فارغاً أبداً. (وقع ٢٠٢٦-٠٨-٢٢ بعد أوّل دورةِ تنظيفٍ وزرع.)
	//
	// **وهي واحدةٌ لا غير** — فهرسٌ فريدٌ يحرسها، فيُنزع الوسمُ عمّا سواها
	// قبل أن يُوضع.
	if _, err := tx.Exec(ctx,
		`UPDATE wallets SET is_treasury = false WHERE is_treasury`); err != nil {
		log.Fatal(err)
	}
	if _, err := tx.Exec(ctx, `
		UPDATE wallets SET is_treasury = true
		 WHERE user_id = (SELECT id FROM users WHERE phone = '+963999000001')`); err != nil {
		log.Fatal(err)
	}
