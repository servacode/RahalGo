...
func TestPL11_13_OrdersBlockedAndNotBypassable(t *testing.T) {
	...
	// ══════════════════════════════════════════════════════════════
	// **والتسجيلُ كذلك — ويُطلَب رمزُه بعينه لا مجرَّدُ ردّ**
	// ══════════════════════════════════════════════════════════════
	//
	// **وكان الشرطُ `Code < 400` فمرّ فارغاً**: **نُزع حارسُ التسجيل
	// في شاهدٍ سالبٍ فبقي الفحصُ أخضر** — **لأنّ البابَ يردّ لأسبابٍ
	// أخرى** (تحقّقُ الحقول) **فيُقرأ الردُّ منعَ إطلاقٍ وليس منه.**
	//
	// **فيُطلَب `launch_closed` بعينه** — **ورمزٌ آخرُ يعني أنّ المنعَ
	// جاء من غير بوّابة الإطلاق.** (قِيس ٢٠٢٦-٠٩-١٦.)
	// **والبابُ `signup/request` لا `signup`** — **والمسارُ الخطأُ
	// يردّ ٤٠٤ فيُقرأ منعاً وهو غياب.** (وقع في أوّل صياغةٍ لهذا الفحص.)
	signup := hh.POST("/api/v1/auth/signup/request", "", map[string]any{
		"phone": "+963900123456",
	})
	if !isLaunchClosed(signup) {
		t.Errorf("**التسجيلُ لم يُردّ ببوّابة الإطلاق**: %d %s — "+
			"**والزرُّ المخفيُّ ليس منعاً.**", signup.Code, signup.Err())
	}
}
