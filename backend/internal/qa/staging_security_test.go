...
func TestFAIL_D13_MediaDirectoryListingOpen(t *testing.T) {
	...
	// **والسردُ أخطرُ من الملفّ**: **من فتح `/media/` رأى أسماءَ كلّ
	// ما رُفع** — وفيها إثباتُ تسليمٍ لطلباتِ أناسٍ آخرين. **ولا
	// يحتاج أن يُخمّن اسماً واحداً.**
	listing := h.GET("/media/", "")
	anon := h.GET("/media/does-not-exist.jpg", "")

	body := string(listing.Body)
	t.Logf("D13 EVIDENCE — سردُ /media/ = %d · ملفٌّ مجهولٌ بلا توكن = %d · جسمٌ %d بايت",
		listing.Code, anon.Code, len(listing.Body))

	if listing.Code != http.StatusOK {
		t.Fatalf(`D13 تبدّل — **سردُ المجلَّد صار مغلقاً** (الردّ %d).

  **وهذا خبرٌ حسن** — فيُحدَّث سجلُّ العيوب ويُحذف هذا الاختبار.`, listing.Code)
	}
	// **وصفحةُ سردٍ لا ملفّ** — «pre» علامةُ سردِ `http.FileServer`.
	if !strings.Contains(body, "<pre>") {
		t.Fatalf("D13: ردَّ 200 بلا صفحةِ سرد — **الدليلُ تبدّل**:\n%s", body)
	}

	t.Logf(`D13 DEFECT REPRODUCED — **سردُ /media/ مكشوفٌ بلا توكن**

  **الردّ 200 وصفحةُ سردٍ كاملة** — **والمجلَّدُ هنا فارغٌ لأنّها بيئةُ
  تجهيزٍ جديدة**، **وعلى قرصٍ فيه ملفّاتٌ يُسرَد كلُّ اسمٍ فيه.**

  **وفيها إثباتُ تسليمٍ لطلباتِ أناسٍ آخرين** — **ولا يحتاج من أرادها
  أن يُخمّن اسماً واحداً.** («XG-21» · «D13».)

  **ولا يُصلَح هنا** — «P-0» تُثبت الدليلَ لا تعالجه.`)
}
