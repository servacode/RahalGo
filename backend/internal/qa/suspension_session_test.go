...
func issuedRefreshMode(t *testing.T, h *Harness, userID, client string, strict bool) (raw, sid string) {
	...
	// ══════════════════════════════════════════════════════════════
	// **وعلَمُ «بدّلها» يُنزَل — والمطلوبُ جلسةٌ لا حسابٌ مقيَّد** (`TMP`)
	// ══════════════════════════════════════════════════════════════
	//
	// **والمِسنَدُ يصنع جلستَه بإعادةِ كلمةٍ إداريّة** — **وهي ترفع
	// `must_change_password` بحقّ**: كلمةٌ وضعها ثالث.
	//
	// **ويومَ صار العلَمُ قيداً في `RequireAuth` (٢٠٢٦-٠٩-١٣) صار كلُّ
	// من يمرّ من هنا مقيَّداً** — **فسقط `R15/A5` في مقدّمته** وهو لا
	// يقيس الكلماتَ المؤقّتة بل سحبَ الأدوار.
	//
	// **فيُنزَل العلَمُ كما يُنزله تبديلُ صاحبِه** — **والقيدُ نفسُه
	// يحرسه `TMP1..4`، فلا يُخفيه هذا.**
	if _, err := h.Pool.Exec(ctxBG(),
		`UPDATE users SET must_change_password = false WHERE id = $1::uuid`,
		userID); err != nil {
		t.Fatalf("إنزالُ علَم التبديل: %v", err)
	}
	return raw, sid
}
