...
// أوّلُ اختبارٍ طلب رابطَ واتساب (`MDIS-010`، ٢٠٢٦-٠٨-٢٠).
//
// # ثمّ ضاقت المساحةُ فتصادمت
//
// **وأُصلحت بـ`%08d` فصار المدى مئةَ مليون** — **وقاعدةُ الاختبار
// تراكم مستخدميها**: بلغوا سبعةً وستّين ألفاً في يوم، **فوقع أوّلُ
// تصادمٍ على `users_phone_key`** (٢٠٢٦-٠٨-٢٠).
//
// **وعطبٌ يظهر بعد ألوفٍ من الصفوف لا يظهر في أوّل تشغيل** — وهذا
// أخبثُ ما فيه.
//
// # فالمدى عشرةُ أرقامٍ لا ثمانية
//
// **وE.164 تسمح بأربعةَ عشرَ رقماً بعد الزائد** (`^\+[1-9][0-9]{7,14}$`)
// — **و`+9639` وعشرةٌ تعطي أربعةَ عشر**، وهو أقصى ما يُسمح.
//
// **والمدى عشرةُ مليارات** — فمليونُ صفٍّ يبقى احتمالُ تصادمه ضئيلا.
//
// **والأساسُ يُختار مرّةً لكلّ عمليّة** والعدّادُ يزيد عليه: **فلا
// تصادمَ داخل التشغيل الواحد أبداً**، والتصادمُ بين تشغيلين يحتاج أن
// يقع الأساسان في مدىً واحد.
// **ومئةُ مليونٍ لا عشرةُ مليارات** (٢٠٢٦-٠٩-١٣): **المحمولُ السوريُّ
// `+963` وتسعةٌ تبدأ بـ9** — **وما زاد ليس رقماً سوريّاً ولو قبلته
// E.164.** **ومُعامَلٌ لا يقبله المنتَجُ لا يقيس المنتَج.**
var phoneBase = time.Now().UnixNano() % 1e8

func uniqPhone() string {
	n := (phoneBase + seq.Add(1)*7) % 1e8
...
type Harness struct {
	...
	Pool *pgxpool.Pool
	Srv  *httptest.Server
	// Orders خدمةُ الطلبات — **لنداءِ ما لا مسارَ شبكةٍ له**، كجولةِ
	// إنذارِ الراصد.
...
// # ما كان يقع
//
// **`fanOut` يأخذ الخمسين الأقدمَ** من `notifications WHERE push_pending`
// (`deliveryBatch`). **وفي الإنتاج تدور نبضةٌ كلَّ ثلاثين ثانيةً
// فتستهلك الصفَّ أوّلاً بأوّل** — **وفي الاختبار لا نبضةَ أصلاً**:
// `RunPushDeliveryWorker` لا يناديها إلّا `cmd/api`.
//
// **فيتراكم المعلَّق**: **٣٨٦٥ إشعاراً في نهاية `internal/qa`** مقيسةً
// على `ed8b43b5`. **وحين يتجاوز المتراكمُ خمسين لا تبلغ جولةُ الفحصِ
// إشعارَه أبداً** — **فلا صفَّ نقلٍ ولا نداءَ ناقل.**
//
// # والمالكُ هو المنتِج لا من يليه
//
// **القاعدةُ التي يعمل بها هذا المِسنَد**: **من أنشأ عملاً مشترَكاً
// دائماً استهلكه أو نظّفه قبل أن يراه غيرُه.**
//
// **فالتنظيفُ في خاتمة الفحص لا في مُفتَتَح ما بعده** — **وفرقُ ذلك
// ليس ذوقاً**: **فحصٌ يزرع معلَّقاً لنفسه ثمّ يبني مِسنَداً ثانياً
// كان يفقد زرعَه**، **والآن لا يفقده** — **ولا تُمسّ إلّا حالٌ صارت
// بلا صاحب.**
//
// # ولماذا الإطفاءُ صادق
//
// **`push_pending = false` لا تعني «فُرِّع فسُلِّم»**: **`fanOutOne`
// تطفئها في معاملتها ولو لم يكن للحساب جهازٌ واحد** — **فصفٌّ بلا
// أهدافٍ حالٌ صحيحةٌ في المنتَج** (وقيس في القاعدة: ألوفٌ منها).
//
// **وعملٌ خلّفه فحصٌ انتهى لا منفّذَ له في هذه العمليّة** — **فبقاؤه
// «معلَّقاً» هو الكذب، لا إطفاؤه.**
//
// # وحصريّةُ المِسنَد شرطُ صحّةِ هذا
//
// **لا `t.Parallel` في المستودع كلِّه** (صفرٌ مقيس)، **وقفلُ جلسةٍ في
// `testdb` يمنع ثنائيّتين على القاعدة نفسِها.** **فلا مِسنَدَ حيٌّ
// لغير هذا الفحص لحظةَ الخاتمة** — **ويحرس ذلك
// `TestXG41C_HarnessExclusivityIsEnforced`.**
//
// **ولا يُمَسّ منتَجٌ**: `deliveryBatch` كما هو، والنبضةُ كما هي.
func ownPushQueue(t *testing.T, pool *pgxpool.Pool) {
	t.Helper()
	t.Cleanup(func() {
		if _, err := pool.Exec(context.Background(),
			`UPDATE notifications SET push_pending = false WHERE push_pending`); err != nil {
			t.Errorf("qa: تعذّر تنظيفُ صفّ الدفع الذي خلّفه هذا الفحص: %v", err)
		}
	})
}
