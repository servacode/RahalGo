...
func build(t *testing.T, pool *pgxpool.Pool, opts ...server.Option) *Harness {
	...
	seedZone(t, pool)
	disownPendingPush(t, pool)
	return &Harness{T: t, Pool: pool, Srv: ts, API: s, Orders: ordersSvc, Hub: hub, MediaDir: mediaDir, tokens: tokens, rdb: rdb}
}

// ══════════════════════════════════════════════════════════════════════
// **وصفُّ الدفع لا يُورَث** — `XG-41C`
// ══════════════════════════════════════════════════════════════════════
//
// # ما كان يقع
//
// **`fanOut` يأخذ الخمسين الأقدمَ من `notifications WHERE push_pending`**
// (`deliveryBatch`، **مقيسٌ في الشيفرة**). **وفي الإنتاج تدور نبضةٌ كلَّ
// ثلاثين ثانيةً فتستهلك الصفَّ أوّلاً بأوّل.** **وفي الاختبار لا نبضةَ
// أصلاً**: `RunPushDeliveryWorker` لا يناديها إلّا `cmd/api`.
//
// **فكلُّ إشعارٍ يولَد في اختبارٍ لا ينادي جولةً يبقى معلَّقاً إلى آخر
// الحزمة.** **وقيس على `ed8b43b5`: ٣٨٦٥ إشعاراً معلَّقاً في نهاية
// `internal/qa`.**
//
// **فحين يبلغ المتراكمُ خمسين، لا تبلغ جولةُ الفحص إشعارَه أبداً** —
// **فلا صفَّ نقلٍ ولا نداءَ ناقل**، ويُقرأ ذلك «الدفعُ لا يعمل».
// **وقيس مباشرةً: `older_pending=86` · `fanned=50` · `صفوفٌ=0`.**
//
// # ولماذا هنا لا في `PF-09`
//
// **الصفُّ موردٌ عامّ**: **من قرأه ملكه.** **وتنظيفُ `PF-09` لصفّ
// غيرِه يمرّ بالناقل فيستهلك ردّاً مبرمَجاً لاختبارٍ لم يبدأ بعد.**
//
// **والعلامةُ تُطفأ ولا يُختلَق تسليم**: **عملٌ خلّفه اختبارٌ انتهى
// لا منفّذَ له في هذه العمليّة** — **فبقاؤه «معلَّقاً» هو الكذب، لا
// إطفاؤه.** **ولا يُمَسّ إشعارُ الاختبار الحاليّ**: هذه تسبقه.
//
// **ولا يُمَسّ منتَجٌ**: `deliveryBatch` كما هو، والنبضةُ كما هي.
func disownPendingPush(t *testing.T, pool *pgxpool.Pool) {
	t.Helper()
	if _, err := pool.Exec(context.Background(),
		`UPDATE notifications SET push_pending = false WHERE push_pending`); err != nil {
		t.Fatalf("qa: تعذّر التبرّؤ من صفّ الدفع الموروث: %v", err)
	}
}
