...
func TestSRV9_PricingReadsTheSamePoint(t *testing.T) {
	...
	for _, c := range []struct {
		...
		// ══════════════════════════════════════════════════════════
		// **والحقلُ الذي يقرؤه العميلُ المنشور** (٢٠٢٦-٠٩-١٣)
		// ══════════════════════════════════════════════════════════
		//
		// **وتطبيقُ الزبون يُعطّل زرَّ الإتمام على `out_of_zone`**
		// (`CartScreen.kt:363`) **ويعرض تحذيراً** (`:328`) — **والمحرّكُ
		// لم يكن يُرسله**، فالتحذيرُ لا يُعرَض والزرُّ لا يُعطَّل.
		//
		// **وتجربةٌ مبنيّةٌ لا تعمل أسوأُ من تجربةٍ لم تُبنَ.**
		wantOut := c.want == "out_of_zone"
		gotOut := strings.Contains(body, `"out_of_zone":true`)
		if gotOut != wantOut {
			t.Errorf("**out_of_zone=%v على %s والمنتظَرُ %v** — "+
				"**وهو ما يُعطّل زرَّ الإتمام في السلّة**", gotOut, c.why, wantOut)
		}
