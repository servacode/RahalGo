...
func TestCoverage_EmptyTableStaysOpen(t *testing.T) {
	h := New(t)
	isolateZones(t, h)
	// **ولا منطقةَ مفعَّلةٌ الآن** — **والقرارُ المكتوب**: جدولٌ فارغٌ
	// ليس «لا نوصّل إلى أحد» بل «لم تُرسَم خريطةٌ بعد».
	if !servable(t, h, 35.9506, 39.0094) {
		t.Error("جدولٌ فارغٌ أغلق التوصيلَ — والقرارُ المكتوبُ يفتحه")
	}
}
