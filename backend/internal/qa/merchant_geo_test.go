...
func TestGEO1_EveryMerchantWritePathDerivesCity(t *testing.T) {
	...
	for _, p := range paths {
		...
		for i, part := range strings.Split(src, "INSERT INTO merchants")[1:] {
			...
			// ══════════════════════════════════════════════════════
			// **وذكرُ العمود ليس اشتقاقَ المدينة**
			// ══════════════════════════════════════════════════════
			//
			// **وكُشف هذا بشاهدِ عزلٍ على هذا الحارس نفسِه**
			// (٢٠٢٦-٠٩-١٧): **وُضع `NULL` مكانَ التعبير فمرّ الحارسُ
			// أخضرَ** — **والسوقُ تختفي عن كلّ زبونٍ يُعرَف موضعُه.**
			//
			// **وحارسٌ يقبل `NULL` يحرس هجاءَ العمود لا معناه.**
			if !strings.Contains(head, "CityOfPointSQL") {
				t.Errorf("**%s: إدراجُ متجرٍ رقم %d يذكر `city_id` ولا يشتقّها** — "+
					"**وعمودٌ يُملأ بـ`NULL` كعمودٍ لم يُذكر**: "+
					"**`NULL = x` لا تصحّ، فيسقط المتجرُ من كلّ ترشيحِ مدينة.**", p, i+1)
			}
		}
