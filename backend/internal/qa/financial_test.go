...
func TestFIN_CommissionSourceMatrix(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			// **والصيغةُ من `settleRep:955`**: يخرج إن كانت عمولةُ المنصّة
			// صفراً — **فالهامشُ وحدَه لا يُعطي المندوبَ شيئاً.**
			want := int64(0)
			if platform != 0 {
				want = margin * int64(c.repP) / 100
			}
			if repCom != want {
				t.Errorf("عمولةُ المندوب %d — والصيغةُ المقيسةُ تعطي %d", repCom, want)
			}
			if platform == 0 && margin > 0 && repCom == 0 {
				t.Logf("GAP (XG-13) — PRICING_MARGIN_ONLY غيرُ مدعوم: هامشٌ %d "+
					"ولا عمولةَ مندوبٍ لأنّ عمولةَ المنصّة صفر", margin)
			}
...
func TestFIN_SnapshotVsLiveEconomics(t *testing.T) {
	...
	// **والعمولةُ تُقرأ حيّةً** — نسبةُ المتجرِ صارت ٤٠٪ بعد الإنشاء.
	margin := (unit - mp) * 1
	live := mp * 40 / 100
	old := mp * 10 / 100
	t.Logf("LIVE-READ — عمولةُ المنصّةِ المقيَّدةُ = %d · بنسبةِ الإنشاء(10%%) = %d · بالنسبةِ الحيّة(40%%) = %d",
		platform, old, live)
	if platform == live && old != live {
		t.Logf("EXPECTED_FAIL (XQ-2 · XG-13) — FINANCIAL SNAPSHOT = NOT IMPLEMENTED: " +
			"طلبٌ أُنشئ بنسبةِ 10%% سُوِّي بنسبةِ 40%%")
	} else if platform == old {
		t.Errorf("العمولةُ لُقطت — وXQ-2 يقول إنّها تُقرأ حيّةً. **يُراجَع.**")
	}
	t.Logf("LIVE-READ — عمولةُ المندوبِ = %d (الهامشُ %d · النسبةُ الحيّةُ 50%%)", repCom, margin)
}
