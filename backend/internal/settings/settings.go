...
//
// **ومَسبَحٌ فارغٌ يُحفَظ فارغاً** — **ولا يُلَفّ في واجهةٍ غيرِ
// فارغة**: `s.db == nil` بعدها تكذب، وهو الفخُّ الذي يحذّر منه
// `Get` نفسُه.
...
// On نسخةٌ تقرأ من منفّذٍ آخر — **معاملةً كان أو مَسبَحاً** — `XG-46`.
//
// # لماذا
//
// **إعدادٌ يُقرأ داخلَ معاملةِ إنشاءِ طلبٍ كان يطلب وصلةً ثانية** —
// **والطلبُ ممسكٌ بالأولى.** **فبمَسبَحٍ سقفُه عشرون، عشرون طلباً
// متزامناً تمسك العشرين ثمّ ينتظر كلٌّ منها وصلةً لا تُفكّ.**
//
// **ولا نسخةَ ثانيةً من الشيفرة**: **الاستعلاماتُ هي هي**، والمبدَّلُ
// من يُنفّذها.
func (s *Store) On(q dbtx.Querier) *Store {
	if s == nil || q == nil {
		return s
	}
	return &Store{db: q}
