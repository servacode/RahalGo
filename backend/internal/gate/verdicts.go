...
var ExternalReqs = []ExternalReq{
	...
	// ── بيئةُ التجهيز · `P-0` (البند ٢١) ──────────────────────────
	{
		...
		Why: "**عُقدةٌ واحدةٌ لا تُثبت الإبطالَ الموزَّع** — والنشرُ عُقَد. " +
			"✅ **والوجهُ الآخرُ من `R16` أُغلق** (دورةُ إصلاحٍ ١٦): " +
			"**سقوطُ `Redis` كان يجعل التوثيقَ يسقط مفتوحاً**، " +
			"**وصارت القاعدةُ هي الحقيقةَ و`Redis` مُسرِّعَ رفض** " +
			"(`TestR16_STG_RedisOutageAuthorityHolds`). " +
			"**ويبقى الإبطالُ الموزَّعُ سؤالاً قائماً**: عُقدتان " +
			"تشتركان في `Redis` — **ولم يُقَس.**",
		Blocking: true, Waiver: WaiverOwnerOnly,
	},
	{
		...
		// **نُشرت فعلاً على `CX33` ٢٠٢٦-٠٩-٠٦** — والأدلّةُ مقيسة.
		Status: Pass,
		Why: "**منشورةٌ وتعمل على `195.201.141.130`**: أربعُ حاوياتِ تجهيزٍ " +
			"معزولةٍ تماماً (`rahalgo-staging_*`) · هجراتٌ نظيفةٌ ١٢٨/١٢٨ · " +
			"حرّاسُ إنتاجٍ ٥/٥ · بذرٌ اصطناعيٌّ · إعادةُ ضبطٍ · تدريبُ نسخٍ " +
			"واستعادةٍ بخطواته · اثنتا عشرةَ طبقةً من خريطة العمليات تردّ " +
			"`200` · هويّةٌ تقول البيئةَ والالتزامَ والهجرة. " +
			"⚠️ **وعلى مضيف الإنتاج نفسِه باستثناءٍ مؤقَّت** — " +
			"`TQ-4-EXCEPTION.md`، **ويُعاد تقييمُه عند أوّل مستخدمٍ حقيقيّ.** " +
			"**و`DNS/TLS` معلَّقان** — لا نطاقَ بعد.",
		Blocking: true, Waiver: WaiverOwnerOnly,
	},
	{
