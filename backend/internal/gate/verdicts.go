...
var ExternalReqs = []ExternalReq{
	...
	// ── بيئةُ التجهيز · `P-0` (البند ٢١) ──────────────────────────
	{
		...
		// ══════════════════════════════════════════════════════════
		// **وقيس على عُقدتين** — دورةُ إصلاحٍ ٢٩، ٢٠٢٦-٠٩-٠٨
		// ══════════════════════════════════════════════════════════
		//
		// **عمليّتان مستقلّتان** (`:8091` و`:8092`، معرّفا عمليّةٍ
		// مختلفان) **تشتركان في قاعدةٍ واحدةٍ وذاكرةٍ واحدة** —
		// **وكلُّ نداءٍ يُوجَّه إلى عنوانٍ بعينه**، فلا يُستنتَج
		// تعدّدُ العُقَد من تكرار نداء.
		//
		// **والمصفوفةُ في `TestSTG01_*`** — تُعاد بأمر واحدٍ لمن
		// ملك الطوبولوجيا، **ولا دليلَ يُكتب بيدٍ في وثيقة.**
		Status: Pass,
		Why: "**قيس الإبطالُ الموزَّعُ على عُقدتين** — إبطالٌ على `A` " +
			"يُردّ على `B` وبالعكس · **وغيابُ مفتاح التسريع لا يُحيي " +
			"جلسة** · **وسقوطُ الذاكرة لا يفتح باباً**: إبطالٌ وقع " +
			"وهي ساقطةٌ طُبِّق، **وعودتُها بلا مفتاحٍ لم تُحيه.** " +
			"**والقاعدةُ هي الحقيقة و`Redis` مُسرِّعُ رفض.** " +
			"**والفروقُ محفوظةٌ عبر العُقَد**: `R13` تقتل كلَّ جلسة · " +
			"`XG-40` تُبقي الجارية وتقتل أخواتها · **والحظرُ يقطع " +
			"والتعليقُ حدُّ تخويلٍ لا قطعُ جلسة** (`XG-39`). " +
			"**ومصافحةُ بثٍّ جديدةٌ تتبع الحقيقةَ نفسَها** — و`R14` " +
			"عقدٌ منفصلٌ لم يُمَسّ.",
		Blocking: true, Waiver: WaiverOwnerOnly,
	},
	{
	...
	},
	{
		...
		Source:      "`AQ-1` · `ADG-1` + `ADG-2` + `ADG-3`",
		Severity:    Critical,
		Evidence:    "موظّفُ تشغيلٍ يُمنَع من تبديل إعدادٍ ماليٍّ حسّاسٍ **من الخادم**",
		// ══════════════════════════════════════════════════════════
		// **وأعضاءُ `AQ-1` الثلاثةُ أُغلقت** — دوراتُ ٢٤ · ٢٥/٢٦ · ٢٨
		// ══════════════════════════════════════════════════════════
		//
		// **`ADG-1`** معجمُ قدرةٍ مُعرَّفٌ في الشيفرة وإسنادٌ في
		// القاعدة، **والافتراضُ منع** ·
		// **`ADG-2`** تسعةُ أدوارٍ وسياسةٌ مركزيّةٌ لكلّ مسار،
		// **و«يقرأ فقط» ليست «أقلَّ صلاحيّة»** ·
		// **`ADG-3`** تأكيدُ الفعل الشديد بكلمةِ صاحبه.
		//
		// **والدليلُ المطلوبُ بنصّه مقيسٌ بنداءٍ حيّ**:
		// `TestADG2_RoleMatrix` — **`operations` يُردّ عن
		// `settings.financial.manage` من الخادم** لا من الشاشة.
		//
		// # ويبقى بندٌ تشغيليٌّ مسجَّل
		//
		// **ترحيلُ حسابات الإنتاج إلى الأدوار التسعة** — **وهو عملُ
		// حساباتٍ لا عقدُ شيفرة**، ولم يُمَسّ إنتاجٌ في أيّ دورة.
		Status: Pass,
		Why: "**ثلاثةُ شروطٍ في الخادم**: قدرةٌ من القاعدة (`ADG-1`) · " +
			"سياسةٌ مركزيّةٌ لكلّ مسارٍ بلا استثناءٍ صامت (`ADG-2`) · " +
			"وتأكيدٌ بكلمةِ صاحبه لأربعةَ عشرَ فعلاً شديداً (`ADG-3`). " +
			"**والقياسُ نداءٌ حيّ**: `TestADG2_RoleMatrix` · " +
			"`TestADG3_HighRiskMatrix` · `TestADG1_D11D12D13_SensitiveRoutesNeedCapability`. " +
			"**ويبقى ترحيلُ حسابات الإنتاج بنداً تشغيليّاً مسجَّلاً.**",
		Blocking: true, Waiver: WaiverOwnerOnly,
	},
