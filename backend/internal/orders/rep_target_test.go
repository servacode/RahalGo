...
// ══════════════════════════════════════════════════════════════════════
// **⚠️ شائخٌ — يحرس عقداً أُلغي** (`STALE / CONTRACT-MISMATCHED`)
// ══════════════════════════════════════════════════════════════════════
//
// **وكلُّ ما فوق هذا السطر كان صحيحاً يومَ كُتب — ٢٠٢٦-٠٨-٣٠.**
//
// **وبدّل المالكُ العقدَ في اليوم التالي** (٢٠٢٦-٠٨-٣١، التزام `5da3dbed`):
//
//	«الهدفُ الشهريّ هو عددُ العملاء المسجَّلين»
//
// **فصار هدفُ المندوب يعدّ ما فتح من متاجر لا طلباتِ متاجره المسلَّمة**،
// **وحُذف النداءُ من مسار التسليم عمداً.** وفي موضعه اليومَ تعليقٌ صريح
// (`orders/transitions.go:346`):
//
//	«ولا هدفَ للمندوب هنا — ومكانُه تبدّل… ونداءٌ لا يغيّر شيئاً أسوأُ
//	 من غيابه… وموضعُه الآن حيث يقع الفعل: grantSalesTargetIfAny عند
//	 إنشاء المتجر.»
//
// **واسمُ هذا الاختبار نفسُه يسمّي المِشغَلَ الملغى**: `GrantedOnDelivery`.
//
// # ولماذا يُتخطّى ولا يُحذَف
//
// **سقوطُه كان يُقرأ عيبَ منتجٍ وليس كذلك** — **واختبارٌ أحمرُ دائماً
// يُطفَأ في الرؤوس قبل أن يُطفَأ في الشيفرة**، فيموت معه ما حوله.
//
// **ولا يُحذَف**: **الأثرُ التاريخيُّ يُحفَظ** (قرارُ المالك ٢٠٢٦-٠٩-٠٥)
// — من قرأ هذا الملفَّ غداً عرف أنّ مِشغَلاً كان ثمّ أُلغي، **ولمَ.**
//
// **والعقدُ الذي حلّ محلَّه يحرسه**: `TestFIN_TargetRewardPrecedesCommit`
// و`TestRACE_DuplicateLeadConversion` — **عند التحويل لا عند التسليم**،
// وهو ما تحرسه `XQ-3`.
//
// **ووُسم في `TEST_TRUTH` بـ`PurposeStale`.**
func TestRepTarget_GrantedOnDelivery(t *testing.T) {
	t.Skip("STALE / CONTRACT-MISMATCHED — هدفُ المندوب صار يُمنَح عند التحويل " +
		"لا عند التسليم (قرارُ المالك ٢٠٢٦-٠٨-٣١) · ويحرسه اليومَ " +
