...
// qaSnapStoreX يُفتح مرّةً على قاعدة الاختبار — **ولا يُبنى مسبحٌ لكلّ نداء.**
var (
	qaSnapStoreXOnce sync.Once
	qaSnapStoreX     *settings.Store
)

func qaSnapStoreXOf() *settings.Store {
	qaSnapStoreXOnce.Do(func() {
		dsn := os.Getenv("TEST_DATABASE_URL")
		if dsn == "" {
			return
		}
		if pool, err := pgxpool.New(context.Background(), dsn); err == nil {
			qaSnapStoreX = settings.NewStore(pool)
		}
	})
	return qaSnapStoreX
}

// qaSnapSQLX أربعُ قيمٍ حرفيّةٍ تُدرَج في `VALUES` — **بقيم اللحظة.**
