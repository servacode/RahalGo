...
func TestZoneAt_EmptyTableIsClosed(t *testing.T) {
	...
	// ══════════════════════════════════════════════════════════════════
	// **ونُقض القرارُ بأمر المالك** (٢٠٢٦-٠٩-١٣)
	// ══════════════════════════════════════════════════════════════════
	//
	// **وكان هذا الاختبارُ يشترط أن يُفتح البابُ بجدولٍ فارغ** — وهو
	// قرارُ ٢٠٢٦-٠٨-١٨: «لم تُرسم خريطةٌ بعد» حالُ إعدادٍ لا قرارُ
	// سياسة.
	//
	// **ونقضه المالكُ لإطلاقٍ عامّ**: **إعدادُ تغطيةٍ غائبٌ لا يفتح
	// العالمَ لقبولِ الطلبات.** **ولا تغطيةَ صالحةً ليس «كلُّ مكانٍ
	// مُغطّى».**
	//
	// **والرمزُ مفترقٌ عن «خارجَ التغطية»** — فالزبونُ لا يرى «خارج
	// النطاق» ولا يُبدّل عنوانَه بلا جدوى، **بل «الخدمةُ غيرُ متاحةٍ
	// مؤقّتاً».**
	if _, err := svc.ZoneAt(ctx, db, lat, lng); !errors.Is(err, orders.ErrCoverageUnavailable) {
		t.Fatalf("**بجدولٍ فارغٍ لم يُردّ بحالِ الإعداد**: %v — "+
			"**والإخفاقُ يجب أن يُغلق.**", err)
	}
