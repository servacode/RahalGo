...
type Order struct {
	...
	// ClosedAt **متى انتهى الطلبُ إلى غير رجعة** — تسليماً أو إلغاءً أو
	// رفضاً أو تعذّراً.
	//
	// **كان يُكتب في القاعدة ولا يعود في أيّ ردّ** — والشاشاتُ تقسم به:
	// «جارية» و«سابقة» و«ملغاة». **وحقلٌ غائبٌ في JSON يُقرأ في الواجهة
	// `undefined`**، و`!undefined` صوابٌ — **فكلُّ طلبٍ جارٍ أبداً**،
	// والتبويبان الآخران فارغان دائماً.
	//
	// **ولا خطأ ولا سطرٌ في سجلّ**: شرطٌ يعمل ويُجيب بالخطأ.
	...
	// ProofMocked **أكان موضعُ التسليم مزيَّفاً؟**
	//
	// (قِيس 2026-09-02: لا فحصَ للتزييف في المنصّة كلِّها — لا في
	//  التطبيق ولا في المحرّك.)
	//
	// **وتطبيقاتُ التزييف مجّانيّةٌ في المتجر**: يضع السائقُ موضعَه
	// عند بيت الزبون وهو في بيته، **فيكتب `ProofMeters` «على بعد
	// خمسةِ أمتار» من نقطةٍ اخترعها هو.**
	//
	// **فالحارسُ الذي بُني للحماية يشهد لمن يخدعه** — ما لم يُقَل
	// للمكتب أنّ الشهادةَ مزيَّفة.
	ProofMocked bool `json:"proof_mocked"`
...
type OrderPage struct {
	...
	// ══════════════════════════════════════════════════════════════════
	// **Counts — كم طلباً في كلّ حال، بغضّ النظر عن الحال المختار**
	// ══════════════════════════════════════════════════════════════════
	//
	// (قرارُ المالك ٢٠٢٦-٠٨-١٢: «بطاقاتٌ ذكيّةٌ أعلى صفحة سجلّ الطلبات،
	//  ونُلغي القائمة المنسدلة ونترك خانة البحث» · «هي سجلُّ طلباتٍ لا
	//  علاقة له بالأمور المالية».)
	//
	// # ولماذا لا تتبع مُرشِّحَ الحال
	//
	// **البطاقاتُ تُرشِّح وتُعَدّ في آن.** ولو تبعت الأعدادُ الحالَ
	// المختار **لصارت البقيّةُ أصفاراً بعد أوّل ضغطة** — فيضغط «ملغى»
	// فيرى «مرفوض ٠» و«فشل التوصيل ٠»، **وهي ليست صفراً بل مخفيّة.**
	//
	// **وصفرٌ يكذب أسوأُ من رقمٍ غائب.**
	//
	// # وتتبع البحثَ لأنّه يحدّ الموضوع
	//
	// **من بحث عن متجرٍ يسأل عن طلباته هو** — وتوزيعُ طلبات المنصّة
	// كلِّها جوابٌ عن سؤالٍ لم يُسأل.
	//
	// **وعدداً لا مبلغاً** — بقرار المالك: **هذا سجلُّ ما وقع، والمالُ
	// دفترُه.**
	//
