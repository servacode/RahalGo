...
func TestCommission_IgnoresPaymentMethodAndManagementMode(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			// عمولةُ المنصة ١٠٪ من البضاعة — **لا من الإجمالي**: رسمُ التوصيل
			// أجرُ خدمةٍ تؤدّيها المنصة بسائقها، لا بيعُ المتجر.
			if got := f.platformCommission(t); got != 10_000 {
				t.Errorf("عمولة المنصة = %d، والمتوقّع 10000", got)
			}
			// ونصيبُ المندوب ١٠٪ منها.
