...
func (s *Service) transitionTx(ctx context.Context, actorID string, actorRoles []string, orderID, to, note, failReason string, hook func(context.Context, dbtx.Querier) error) (*Order, error) {
	...
	// ══════════════════════════════════════════════════════════════
	// **وفاعلُ النظام لا اسمَ له** — `PF-08`
	// ══════════════════════════════════════════════════════════════
	//
	// **الكانسُ ينادي بفاعلٍ فارغ** (`sweepAutoAccept` وكنسُ التحويل) —
	// **و`actor_id` من نوع `uuid`، والفارغُ ليس معرّفاً** فيسقط
	// الإدخالُ وتسقط المعاملةُ كلُّها.
	//
	// **فكان الكنسُ القائمُ مكسوراً صامتاً**: يلتقط الطلبَ ثمّ يفشل،
	// **وخطؤه في سجلٍّ لا يقرؤه أحد.** (مقيسٌ ٢٠٢٦-٠٩-٠٧: التُقط=1
	// وتقدّم=0.)
	//
	// **والفارغُ يصير `NULL`** — **وهو ما يعنيه: لا إنسانَ فعلها.**
	// **ولا يُختلَق معرّفٌ لحسابِ نظامٍ وهميّ** — من فعلها النظامُ،
	// والعمودُ يقبل الخلوّ.
	// **والفراغُ يصير `NULL` في `Go` لا في `SQL`**: **`NULLIF` يقلب
	// استنباطَ النوع** — يصير الوسيطُ نصّاً بدل `uuid` — **فتتبدّل
	// خططُ استعلاماتٍ لا علاقةَ لها.** (قِيس: فحصٌ بعيدٌ عَلِق ٦٣ ثانية.)
	var actor any
	if actorID != "" {
		actor = actorID
	}
	...
	// ══════════════════════════════════════════════════════════════════
	// **ولا هدفَ للمندوب هنا — ومكانُه تبدّل**
	// ══════════════════════════════════════════════════════════════════
	//
	// **كان يُنادى هنا** حين كان هدفُ المندوب يعدّ طلبات متاجره
	// المسلَّمة. **وصار يعدّ ما فتح من متاجر** (قرارُ المالك
	// ٢٠٢٦-٠٨-٣١)، **فالتسليمُ لا يحرّك عدّادَه بحرف.**
	//
	// **ونداءٌ لا يغيّر شيئاً أسوأُ من غيابه**: يقرؤه من بعدَنا فيظنّ
	// أنّ للتسليم أثراً في هدف المندوب، **فيبني عليه.**
	//
	// **وموضعُه الآن حيث يقع الفعل**: `grantSalesTargetIfAny` عند إنشاء
	// المتجر — من تحويل الطلب أو من لوحة الإدارة.
	...
	// ══════════════════════════════════════════════════════════════════
	// **وفي وضع المتاجر: القبولُ يدخل التحضيرَ بنفسه**
	// ══════════════════════════════════════════════════════════════════
	//
	// (قرارُ المالك ٢٠٢٦-٠٨-٢٩: «بعدما يتمّ استلامُ الطلب يدخل بمرحلة
	//  التحضير بشكل تلقائيّ، لا داعيَ أن يقول بدأتُ التحضير».)
	//
	// **وصاحبُ المطعم يقبل الطلبَ وهو يقف على النار** — والقبولُ عنده
	// يعني أنّه بدأ. **وزرٌّ ثانٍ يقول ما فعله فعلاً ضريبةُ ضغطةٍ بلا
	// معنى**، ومن نسيه بقي طلبُه «مقبولاً» لا يُنزَل عليه سائق.
	//
	// **وفي وضع المنصّة تبقى الخطوتان**: المكتبُ يقبل، **والمتجرُ هو من
	// يقول متى بدأ** — وهما شخصان لا واحد.
	//
	// **ويُنفَّذ هنا لا في التطبيق**: نداءان متتاليان من الجهاز يقعان
	// نصفَهما على شبكةٍ منقطعة، **فيبقى الطلبُ في حالةٍ لا زرَّ لها.**
	if s.autoPreparing(ctx, to) {
		started, serr := s.Transition(ctx, actorID, actorRoles, orderID, StPreparing, "")
		if serr != nil {
			s.logger.Warn("التحضيرُ التلقائيُّ تعثّر — الطلب يبقى مقبولاً",
				"order", orderID, "error", serr)
		} else {
			return started, nil
		}
	}
...
// autoPreparing **أيدخل الطلبُ التحضيرَ فورَ قبوله؟**
//
// **في وضع المتاجر وحدَه** — انظر النداء.
func (s *Service) autoPreparing(ctx context.Context, to string) bool {
	if s.settings == nil || to != StAccepted {
		return false
	}
	return s.MerchantsSelfManage(ctx)
}
...
type settled struct {
	...
	// ══════════════════════════════════════════════════════════════════
	// **مندوبُ المتجر — ولو لم يقبض عمولةً عن هذا الطلب**
	// ══════════════════════════════════════════════════════════════════
	//
	// **و`repID` أعلاه لا يصلح لهذا**: يُملأ فقط حين تُقيَّد عمولة،
	// **فيبقى فارغاً إن كانت النسبةُ صفراً** — وهي صفرٌ اليوم.
	//
	// ══════════════════════════════════════════════════════════════════
...
func (s *Service) reverseCommissions(ctx context.Context, q wallet.Querier, orderID, actorID string) error {
	...
	// ══════════════════════════════════════════════════════════════
	// **ولا يُشترَط حقُّ الزبون برصيدِ من بعده** — `XG-11`
	// ══════════════════════════════════════════════════════════════
	//
	...
	// # وما كان يقع
	//
	// **كان يُخصم المستحقُّ كاملاً بلا سؤالٍ عن الرصيد** — **فإن كان
	// المتجرُ قد سحبه أسقط `CHECK (balance >= 0)` المعاملةَ كلَّها**:
	// يُردّ النداءُ `409`، **ولا يصل الزبونَ قرشٌ واحد.**
	//
	// **والزبونُ لم يفعل شيئاً** — دفع وانتظر ولم يصله طلبُه، **ثمّ
	// حُرم مالَه لأنّ طرفاً ثالثاً سحب مستحقَّه.**
	//
	// # ولماذا هذا الحلُّ لا غيرُه
	//
	// **الآليّةُ قائمةٌ ومُثبَتةٌ في مسار ردّ البضاعة** (`goods.go:160`):
	// **يُؤخذ ما تحتمله المحفظة، ويبقى الباقي ديناً** يُقتطَع من أوّل
	// مستحقٍّ قادم (`offsetMerchantDebt`). **وهي عينُ الحالة**: مالٌ
	// قُيّد للمتجر عن بيعٍ لم يتمّ.
	//
	// **فالعلّةُ أنّ المسارين افترقا** — أُصلح أحدُهما ونُسي الآخر.
	// **ولا آليّةَ جديدةٌ تُخترَع**: النوعُ نفسُه (`merchant_earning`
	// سالباً) فتراه الخزينة، **والدَّينُ في العمود الذي يعرفه التحصيل.**
	//
	// # ولا يُعفى المتجر
	//
	// **ما عجز عنه رصيدُه يبقى عليه** — **ولا يمحوه عجزُه.**
