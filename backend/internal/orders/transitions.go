...
func (s *Service) TransitionWithReason(ctx context.Context, actorID string, actorRoles []string, orderID, to, note, failReason string) (*Order, error) {
	...
	// ══════════════════════════════════════════════════════════════════
	// **وللمندوب هدفُه كذلك — وكان لا يُنادى قطّ**
	// ══════════════════════════════════════════════════════════════════
	//
	// **`GrantTargetIfReached` تعرف دورَ `sales`** منذ كُتبت: تقرأ
	// `sales.monthly_target` و`sales.target_reward`، **و`doneThisMonth`
	// لها استعلامُها الخاصّ** يعدّ طلبات متاجره المسلَّمة.
	//
	// **ولا موضعَ في المنصّة كلِّها يناديها بـ`"sales"`** — السائقُ
	// وحدَه. **فالشريطُ يمتلئ ويصير أخضرَ ويُكتب «تحقق الهدف» ولا يدخل
	// قرشٌ محفظتَه**، ولا خطأَ يظهر ولا سطرَ في سجلّ.
	//
	// **وعطبٌ لا يشتكي أطولُ عمراً من عطبٍ يصرخ**: يظنّ المندوبُ
	// المكافأةَ متأخّرةً أو أنّه أخطأ في العدّ، **وقد يمضي شهرٌ قبل أن
	// يسأل.**
	//
	// (كشفه جردُ تطبيق المندوب ٢٠٢٦-٠٨-٣٠، وأُصلح بإذن المالك.)
	//
	// **وبعد الإيداع كأختِها**، **والتكرارَ تمنعه القاعدةُ** — فهرسٌ
	// فريدٌ لكلّ شهر.
	if to == StDelivered && done.targetRepID != "" && s.targets != nil {
		s.targets.GrantTargetIfReached(ctx, done.targetRepID, "sales")
	}
...
type settled struct {
	...
	//
	// **ومكافأةُ الهدف لا علاقة لها بالنسبة**: هدفٌ بلغه المندوبُ يُكافأ
	// عليه ولو كانت عمولةُ الطلب الواحد صفراً. **ولو رُبطا لَعُلِّقت
	// المكافأةُ على إعدادٍ آخر بلا أن يقول أحدٌ ذلك.**
	targetRepID string
	// ══════════════════════════════════════════════════════════════════
...
func (s *Service) settleRep(ctx context.Context, q wallet.Querier, orderID, actorID string, out *settled) error {
	...
	// **ويُلتقط المندوبُ هنا قبل أيّ خروج** — لمكافأة الهدف لا للعمولة.
	//
	// **وشراؤه من متجره لا يُحسب له**: عدُّ طلباتٍ اشتراها بنفسه يجعل
	// بلوغَ الهدف بيده، **وهي الحجّةُ نفسُها التي تمنعه من عمولتها.**
	if repID != nil && !repIsBuyer {
		out.targetRepID = *repID
	}
