...
func TestRefundAfterDelivery_ReversesEverything(t *testing.T) {
	...
	// ══════════════════════════════════════════════════════════════
	// 2) **وعمولةُ المندوب تُعكَس** — `RQ-5` · قرارُ المالك ٢٠٢٦-٠٩-٠٥
	// ══════════════════════════════════════════════════════════════
	//
	...
	//
	// # وكان هذا الاختبارُ يؤكّد النقيض
	//
	// **استند إلى قرارٍ بتاريخ ٢٠٢٦-٠٨-٠٣** — «المندوبُ لا علاقة له
	// بذلك» — **وسياقُه نزاعُ الطعام الفاسد**: من يتحمّل الخسارةَ حين
	// يشتكي زبون، **لا كيف يُقفَل دفترُ طلبٍ استُرِدّ.**
	//
	// **والقرارُ الأحدثُ نسخه صراحةً** (صُولحت الوثيقتان ٢٠٢٦-٠٩-٠٦).
	// **ولم يُبدَّل التوكيدُ ليمرّ إصلاح** — **بُدّل لأنّ العقدَ تبدّل**،
	// **والاختبارُ يحرس العقدَ النافذ لا الذي مضى.**
	if got := f.balance(t, f.rep); got != 0 {
		t.Errorf("عمولةُ المندوب = %d بعد الاسترداد، والمتوقّع 0 — **تُعكَس**", got)
	}
	st, err := f.wallet.StatementFor(ctx, f.rep, 10)
	if err != nil {
		t.Fatalf("تعذّر كشف محفظة المندوب: %v", err)
	}
	// **قيدان: العمولةُ وعكسُها** — **ولا يُمحى التاريخ.**
	if len(st.Transactions) != 2 {
		t.Errorf("قيودُ المندوب %d، والمتوقّع 2 (عمولةٌ وعكسُها)", len(st.Transactions))
	}
