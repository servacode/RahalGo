...
type QuoteResult struct {
	...
	// OutOfZone **خارجَ التغطية** — **وهو الحقلُ الذي يقرؤه العميلُ
	// المنشور.**
	//
	// ══════════════════════════════════════════════════════════════
	// **وعقدٌ يقرؤه العميلُ ولا يُرسله المحرّكُ عقدٌ ميّت**
	// ══════════════════════════════════════════════════════════════
	//
	// **وقِيس ٢٠٢٦-٠٩-١٣**: تطبيقُ الزبون يقرأ `out_of_zone` في
	// `Shop.kt:164`، **ويُعطّل زرَّ الإتمام عليه** (`CartScreen.kt:363`)
	// **ويعرض تحذيراً** (`:328`) — **والمحرّكُ لا يُرسله إطلاقاً.**
	//
	// **فالحقلُ `false` أبداً**: **التحذيرُ لا يُعرَض والزرُّ لا يُعطَّل**،
	// ويمضي الزبونُ إلى الإتمام ليُردّ بـ`out_of_zone` هناك.
	//
	// **وتجربةٌ مبنيّةٌ لا تعمل أسوأُ من تجربةٍ لم تُبنَ** — **فمن رآها
	// في الشيفرة حسبها تعمل.**
	//
	// **وهو أخصُّ من `Serviceable`**: **نقطةٌ لم تُحدَّد ليست «خارجَ
	// التغطية»** — فيبقى هذا `false` وذاك `false` بسببٍ آخر.
	...
	// ══════════════════════════════════════════════════════════════
	// **والحالُ كلُّها في حقلٍ واحدٍ يقرؤه من يشاء** (`AV`، ٢٠٢٦-٠٩-١٤)
	// ══════════════════════════════════════════════════════════════
	//
	// **والحقولُ فوقَه تبقى كما هي حرفاً** — **وعميلٌ منشورٌ يقرؤها
	// يعمل كما كان.** **وهذا يُضاف ولا يُبدِّل.**
	//
	// **ويجمع ما كانت الشاشةُ تجمعه بنفسها**: وضعَ الإطلاق ودوامَ
	// المنصّة والجغرافيا الإداريّةَ والتغطيةَ ووقتَ المنطقة ودوامَ
	// المتجر — **وسببٌ واحدٌ يُعرَض، هو أعمُّ ما يمنع.**
...
func (s *Service) ZoneAt(ctx context.Context, q dbtx.Querier, lat, lng float64) (ZoneCharge, error) {
	...
	if len(cands) == 0 {
		...
		// ══════════════════════════════════════════════════════════════
		//
		// (شكوى المالك ٢٠٢٦-٠٨-١٨: «التوصيلُ لا يجلب سعرَ التوصيل من
		//  لوحة الإدارة، تأكّد منه».)
		//
		// **وقِيس على الخادم**: `delivery.fee = 110` مضبوطةٌ في اللوحة،
		// **و`delivery_zones` صفرُ صفوف.** فيردّ `ZoneAt` «خارجَ
		// التغطية»، **وتضع التسعيرةُ الرسمَ صفراً بلا خطأ** — فيرى
		// الزبونُ «التوصيل ٠» ويظنّ أنّ اللوحةَ لا تصل إليه.
		//
		// # ولماذا تُفتح لا تُغلق
		//
		// **«لم تُرسم خريطةٌ بعد» حالُ إعدادٍ لا قرارُ سياسة.** ومنصّةٌ
		// تُقلَع فتردّ كلَّ طلبٍ بـ«خارج نطاق التوصيل» **تبدو معطوبةً
		// لا مضبوطة** — ولا شيءَ يقول أين الخلل.
		//
		// **ومن أراد أن يوقف التوصيل يُطفئ المنصّةَ أو يغلق الأقسام** —
		// **لا يمحو خريطتَه.**
		//
		// # والدوائرُ تحكم متى وُجدت
		//
