...
//
//	أو تلقائيّاً البوت يرسلها — لا SMS».)
//
// **ويُسأل قبل أن يُقبل الطلبُ لا بعده**: **طلبٌ مقبولٌ لا يعلم به
// المتجرُ أسوأُ من طلبٍ معلَّق** — المعلَّقُ يراه الموظّفُ فيرسله بيده،
// **والمقبولُ يقول للزبون «قيد التحضير» ولا أحدَ يطبخ.**
//
// **ولا يُستبدل بمحاولةٍ ثمّ تراجع**: التراجعُ عن قبولٍ وقع يُربك
// الشاشتين معا.
func (s *WhatsAppSender) Ready() bool {
	c, _ := s.cli()
	return c.IsLoggedIn() && c.IsConnected()
}
