...
func Pool(t *testing.T) *pgxpool.Pool {
	...
	// ══════════════════════════════════════════════════════════════════
	// **ومجمّعُ الفحص كمجمّع الإنتاج** (`NT`، ٢٠٢٦-٠٩-١٥)
	// ══════════════════════════════════════════════════════════════════
	//
	// **وافتراضُ `pgx` أربعةُ اتّصالاتٍ أو عددُ الأنوية** — **والإنتاجُ
	// على عشرين** (`database/postgres.go`).
	//
	// **وقِيس**: **تسعيرتان متزامنتان تمرّان وثلاثٌ تعلق حتّى مهلة
	// الخادم** — **والتسعيرةُ تمسك اتّصالين.**
	//
	// **فكلّما كبرت الحزمةُ سقطت فحوصُ التزامن بمهلةِ ثلاثين ثانية** —
	// **وهي تنجح وحدَها.** **وفحصٌ يسقط بحدّ أداته لا بعيبِ منتَجٍ
	// يُعلّم الناسَ ألّا يقرؤوا الأحمر.**
	//
	// **ولا يُخفي هذا عيباً**: **المنتَجُ يُقاس بما يشبه الإنتاج، لا
	// بأضيقَ منه.**
	cfg, err := pgxpool.ParseConfig(url)
	if err != nil {
		t.Fatalf("تعذّر قراءةُ عنوان قاعدة الاختبار: %v", err)
	}
	cfg.MaxConns = 20
