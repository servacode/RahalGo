...
func Pool(t *testing.T) *pgxpool.Pool {
	...
	// ══════════════════════════════════════════════════════════════════
	// **وتُنتظَر الجاهزيّةُ لا يُكتفى بنبضة** — `XG-34`
	// ══════════════════════════════════════════════════════════════════
	//
	// **حاويةٌ أُقلعت للتوّ تردّ `57P03`**: «النظامُ لا يقبل اتّصالاتٍ
	// بعد» — **وهي تُعيد الاستردادَ بعد إيقافٍ سابق.**
	//
	// **ونبضةٌ واحدةٌ تسقط فيسقط الفحصُ برسالةٍ لا تشبه سببَها**:
	// **تعثّرٌ في فحصٍ ماليٍّ بعيدٍ وسببُه أنّ القاعدةَ لم تستيقظ.**
	// (رُصد ٢٠٢٦-٠٩-٠٧ بنصّه.)
	//
	// **فتُعاد المحاولةُ حتّى تستجيب أو تنقضي مهلة** — **وثلاثون ثانيةً
	// تكفي إقلاعَ حاويةٍ محلّيّة**، ومن تجاوزها فعطبُه غيرُ هذا.
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatalf("قاعدة الاختبار لا تستجيب بعد ٣٠ ثانية: %v", err)
		}
	...
	// **ويُطلَق قفلُ التهيئة هنا** — `XG-34`: **حَرَسَ ما يحرسه** (المسحَ
	// والهجرات)، **وإبقاؤه يحبس ثنائيّةَ الحزمة التالية.**
	releaseLock()
