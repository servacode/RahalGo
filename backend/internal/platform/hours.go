...
// NextCloseAt **متى تُغلَق الفترةُ المفتوحةُ الآن — نهايتُها الفعليّة.**
//
// **ونظيرُ `NextOpenAt`**: تلك تقول متى يُفتح لمن هو مغلق، وهذه متى
// يُغلَق لمن هو مفتوح. **وتُقرأ بنفسِ منطقِ `OpenAt`** (صدرُ اليوم،
// وذيلُ أمسِ العابر) فلا تفترقان.
//
// **والعابرةُ منتصفَ الليل تُغلَق غداً**: من كان في فترةٍ تبدأ العاشرةَ
// مساءً وتنتهي الثالثةَ فجراً، **نهايتُها الثالثةُ من الغد لا اليوم** —
// **و«لا إغلاقَ بقيَ اليوم» ليست «لا إغلاقَ له».**
//
// **وذيلُ أمسِ يُغلَق اليوم**: من كان في الساعةِ الواحدةِ ليلاً داخلَ
// فترةٍ بدأت أمسِ، نهايتُها اليومَ عند `End`.
//
// **وفارغٌ يعني «لا إغلاقَ مجدولٌ لحاله الآن»** — **ولا يُنادى إلّا
// حين يكون داخلَ فترة**: من ليس مفتوحاً لا نهايةَ فترةٍ له، ومن لا
// جدولَ ساريَ له مفتوحٌ بلا حدّ. **ولا يُخترَع موعد.**
func (s Schedule) NextCloseAt(now time.Time) *time.Time {
	loc := Location()
	t := now.In(loc)
	day := int(t.Weekday())
	m := Minutes(t.Hour()*60 + t.Minute())
	...
	// **النهايةُ بالتقويم لا بالحساب**: `AddDate` يعبر الشهرَ والسنةَ
	// والأسبوعَ بلا خطأٍ يدويّ.
	at := func(dayOffset int, end Minutes) *time.Time {
		d := t.AddDate(0, 0, dayOffset)
		c := time.Date(d.Year(), d.Month(), d.Day(),
			int(end)/60, int(end)%60, 0, 0, loc)
		return &c
	}
	...
	for _, w := range s {
		switch {
		case w.Day == day && !w.CrossesMidnight():
			// **فترةُ اليومِ العاديّة** — تُغلَق اليومَ عند `End`.
			if m >= w.Start && m < w.End {
				return at(0, w.End)
			}
		case w.Day == day && w.CrossesMidnight():
			// **صدرُ العابرة** — تُغلَق غداً عند `End`.
			if m >= w.Start {
				return at(1, w.End)
			}
		case w.Day == yesterday && w.CrossesMidnight():
			// **ذيلُ عابرةِ أمس** — تُغلَق اليومَ عند `End`.
			if m < w.End {
				return at(0, w.End)
			}
		}
