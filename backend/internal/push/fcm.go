...
func (f *FCM) sendOne(ctx context.Context, endpoint, auth, device string, msg Message) (gone bool, err error) {
	...
	payload := map[string]any{
		"message": map[string]any{
			"token": device,
			// **إشعارٌ وحمولةٌ معاً**: النظامُ يعرض الإشعارَ ولو كان
			// التطبيقُ مقتولاً، **والحمولةُ تقول للتطبيق أين يذهب** حين
			// يُفتح.
			...
			"android": map[string]any{
				...
				"notification": map[string]any{
					// **قناتان لا واحدة**: العاجلُ يرنّ ويهتزّ، والعاديُّ
					// يصمت. **والقناةُ تُنشأ في التطبيق بهذا الاسم بالضبط.**
					"channel_id": channelFor(msg.Urgent),
				},
			},
		},
	}
	...
	case res.StatusCode < 300:
		return false, nil
	// **٤٠٤ رمزٌ لم يعد موجوداً، و٤٠٣ رمزٌ لمشروعٍ آخر** — كلاهما لا
	// يُشفى بإعادة المحاولة، **فيُحذف.**
