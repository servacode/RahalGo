...
func (s *Service) Diagnose(ctx context.Context, userID string) Diagnosis {
	...
	for platform, tokens := range byPlatform {
		...
		// **ورمزٌ رُفض عطبٌ يُقال** — (قِيس ٢٠٢٦-٠٨-١٤: «أُرسلت إلى ٠
		// من ١» وظهرت خضراءَ **لأنّ الصفرَ لم يُفحَص**).
		//
		// **وجوابٌ يقول «وصلت» وهي لم تصل أسوأُ من صمت** — يُغلق البابَ
		// على البحث.
		if len(dead) > 0 && out.Error == "" {
			out.Error = "رفضت غوغل رمزَ الجهاز (غيرُ مسجَّل) — أعد فتحَ التطبيق، فإن تكرّر فالمشروعُ غيرُ مطابق"
		}
	}
	return out
}
