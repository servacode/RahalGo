...
const (
	...
	// ObservabilityRead **قراءةُ صحّة المنصّة الداخليّة** — دورةُ ٧٠أ.
	//
	// **ولا تُستعمل لها `analytics.read`**: **تلك أرقامُ عملٍ —
	// مبيعاتٌ وطلباتٌ وأداءُ سائقين**، **وهذه صحّةُ بنيةٍ: مسبحُ
	// اتّصالاتٍ ومعاملاتٌ عالقةٌ وحصيلةُ دفع.** **ومن جمعهما في قدرةٍ
	// واحدةٍ أعطى محلّلَ المبيعات بابَ التشخيص، أو منع مهندسَ
	// التشغيل من بابه لأنّه لا يقرأ الأرباح.**
	//
	// **وقراءةٌ لا إدارة**: **لا فعلَ خلف هذا الباب** — يُقرأ ولا
	// يُصلَح، **فلا `observability.manage` ما دام لا شيءَ يُدار.**
	ObservabilityRead Capability = "observability.read"
