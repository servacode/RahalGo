...
// GraceAfterDelivery **مهلةٌ بعد التسليم — لا إغلاقٌ لحظته.**
//
// (وهي في وثيقة المالك، وأصابت.) **من اكتشف نقصاً في الطلب يكتشفه وهو يفتح
// الكيس** — لا بعد ساعة. **وإغلاقٌ لحظةَ التسليم يقطع الحديثَ في أحوج لحظاته.**
const GraceAfterDelivery = 15 * time.Minute
...
func channelOpen(status string, deliveredAt, closedAt *time.Time) (bool, *time.Time) {
	...
	// **والمُسلَّمُ يُقاس بوقته لا بحالته.**
	if status == "delivered" && deliveredAt != nil {
		end := deliveredAt.Add(GraceAfterDelivery)
		return time.Now().Before(end), &end
	}
	// **وكلُّ ما عداه مغلق**: مُلغًى، مرفوض، متعذّر، مسترجَع.
	//
	// **والمتعذّرُ مغلقٌ عمداً**: بضاعةٌ لم تُسلَّم خلافُها في العمليات لا بين
	// طرفين — **ومحادثةٌ بلا شاهدٍ تصير كلمتَه ضدّ كلمته.**
	_ = closedAt
	return false, nil
}
