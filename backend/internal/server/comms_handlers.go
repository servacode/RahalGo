...
func (s *Server) handleSendOrderMessage(w http.ResponseWriter, r *http.Request) {
	...
	s.notify.Notify(r.Context(), notifications.Input{
		...
		// ══════════════════════════════════════════════════════════════
		// **والرسالةُ ترنّ ولا تُحفَظ**
		// ══════════════════════════════════════════════════════════════
		//
		// (قرارُ المالك ٢٠٢٦-٠٨-١٢: «رسائلُ المحادثة ما يصير تصل
		//  كإشعارات أيضا — هي أساساً تصل بالرسائل، لماذا تصل كإشعارٍ
		//  أيضا؟».)
		//
		// **والحديثُ نفسُه هو السجلّ**: يُقرأ فيه ما قيل بوقته وترتيبه،
		// **وسطرٌ في صندوق الإشعارات يعيد أوّلَ مئةٍ وعشرين حرفاً** ثمّ
		// يبقى بعد أن قُرئ الأصل.
		//
		// **ولا تُسكَت الرنّة**: من أغلق تطبيقَه لا يعرف أنّ السائق
		// يسأله «أيّ طابق؟» — **فيقف على الباب ينتظر جواباً لا يأتي.**
		Transient: true,
	})
	s.touchUser(p.PeerID, "order")
