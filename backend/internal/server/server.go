	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Route("/auth", func(r chi.Router) {
			...
			r.Group(func(r chi.Router) {
				...
				r.Get("/me", s.handleMe)
				// ══════════════════════════════════════════════════
				// **وقدراتُ صاحب الجلسة — لبابِ القائمة** (٢٠٢٦-٠٩-١٢)
				// ══════════════════════════════════════════════════
				//
				// **وكانت القائمةُ تُبوَّب بأسماء الأدوار** (`roles:
				// ["admin"]`) — **فدورٌ جديدٌ يحمل قدرةً لا يرى بابَها**
				// حتّى يُكتب اسمُه في الواجهة. **وذاك `ADG-1` في العرض:
				// حقيقةٌ ثانيةٌ في العميل تفترق يوماً.**
				//
				// **ولا يكشف للفاعل إلّا ما يملكه هو** — والخادمُ يحسبه
				// في كلّ نداءٍ أصلاً (`capabilitiesFrom`)، **وسابقتُه
				// خريطةُ العمليات تردّ `perms` منذ دورات.**
				//
				// **وإخفاءُ بندٍ لطفٌ بالعين لا حراسة** — والحدُّ في
				// جدول السياسة كما كان.
				r.Get("/capabilities", s.handleMyCapabilities)
		...
		// ══════════════════════════════════════════════════════════
		//
		// **وعامّةٌ بلا توكن**: **من يفتح لوحةَ التجهيز يحتاج أن يعرف
		// أنّها التجهيزُ قبل أن يسجّل دخوله**، **ومن جرّب على الإنتاج
		// ظانّاً أنّه التجهيز لا ينفعه بابٌ يحتاج حساباً.**
		//
		...
		if s.qaStagingEnabled() {
			...
			s.logger.Warn("QA staging endpoints ENABLED — staging only (POST /api/v1/qa/session, /qa/revoke, /qa/setting, /qa/seed)")
		}
		...
		r.Group(func(r chi.Router) {
			...
			//
			// ══════════════════════════════════════════════════════════
			// **ومحميٌّ من الإعادة كأخيه**
			// ══════════════════════════════════════════════════════════
			//
			// (تدقيقُ الإطلاق ٢٠٢٦-٠٨-١٩ — BUG-002.)
			//
			// **وكان مكشوفاً وحدَه**: `/orders` ملفوفةٌ منذ بُنيت وهذه
			// لا. **وبابان لفعلٍ واحدٍ أحدُهما محروسٌ والآخرُ لا** —
			// وهي عائلةُ الخلل نفسُها: قاعدةٌ تُطبَّق في موضعٍ وتُنسى
			// في مثله.
			//
			// **وأشدُّ من أخيه**: لا حمايةَ في الخادم ولا في التطبيق،
		...
		r.Route("/rep", func(r chi.Router) {
			...
			// ══════════════════════════════════════════════════════════
			//
			// **كان داخلَه فردّ ٤٠٣ في كلّ مرّة** (قِيس على الجهاز
			// ٢٠٢٦-٠٨-١٨: المندوبُ يختار صورةً فتقول له الشاشةُ «هذا
			// الحسابُ ليس حساب سائق»).
			//
			// **والحارسُ يسأل عن معرّفٍ في المسار** — `id` أو `itemID`
			// أو `sectionID` — **وهذا البابُ لا معرّفَ فيه**، فيقع في
			// «الافتراضُ منعٌ» فيُردّ.
			//
			// **ولا متجرَ يُحرَس هنا أصلاً**: الرفعُ يُنشئ صفَّ وسيطٍ لا
			// يخصّ أحداً، **والربطُ يقع في نداءِ الصنف** — وذاك محروس.
			// **ودورُ `sales` يكفيه** كما يكفي رفعَ صورة الحساب.
		...
		r.Route("/admin", func(r chi.Router) {
			...
			// **ورابعُها تشكيلُ الحمولة** — `XG-42`: **البابُ ليس
			// الحمولة**، **ومن بلغ المسارَ لا ينال كلَّ حقولِه.**
			// **وبعد التخويل لأنّ القدراتِ تُقرأ من سياقه.**
			r.Use(s.shapeResponse)
			...
			// ══════════════════════════════════════════════════════
			// **وصحّةُ المنصّة الداخليّة** — دورةُ ٧٠أ
			// ══════════════════════════════════════════════════════
			//
			// **وتحت السطح الإداريّ لا بجانبه**: **فيرث أربعةَ حرّاسٍ
			// قائمين** — جلسةٌ، وقدرةٌ ما، وسياسةُ الجدول
			// (`observability.read`)، وتشكيلُ الردّ. **ومن بنى له
			...
			//
			// **والوسائطُ الشخصيّةُ تأتي موقَّعةً في ردودها أصلاً**
			// (إثباتُ التسليم في الطلب · الصورةُ في الملفّ)، **وهذه
			// لمن بيده مسارٌ ويريد رابطاً حيّاً** — تدقيقٌ أو نزاع.
			r.Get("/media/sign", s.handleSignMedia)
