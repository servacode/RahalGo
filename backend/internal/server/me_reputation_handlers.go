...
func (s *Server) handleMeReputation(w http.ResponseWriter, r *http.Request) {
	...
	crows, err := s.pg.Query(ctx, `
		...
		WHERE `+ownerCond+complaintCond+`
		  ...
		  -- ══════════════════════════════════════════════════════════
		  -- **وما كتبتُه أنا لا يكون شكوى عليّ**
		  -- ══════════════════════════════════════════════════════════
		  --
		  -- (شكوى المالك ٢٠٢٦-٠٨-١٣: «أبلغتُ على زبونٍ ومتجر، وفُهم
		  --  البلاغُ على أنّه ضدّي».)
		  --
		  -- **والشرطُ فوقه يكفي لما يُفتح اليوم** — الجهةُ تُكتب عند
		  -- الفتح. **وهذا للقديم**: صفوفٌ فُتحت قبل الإصلاح جهتُها
		  -- فارغة، **وفارغُها يُعرض للجميع.**
		  --
		  -- **وشرطٌ لا يعتمد على عمودٍ يُملأ صحيحاً أمتنُ من شرطٍ
		  -- يعتمد عليه**: من كتب شكوى لا يُتّهم بها، مهما قالت
		  -- الأعمدةُ الأخرى.
