...
func TestAnnounceWrites_IsWiredToBothGates(t *testing.T) {
	...
	for _, gate := range []string{`r.Route("/admin"`, `r.Route("/merchant"`} {
		...
		// في هذه.**
		//
		// **واتّسعت النافذةُ في دورةِ ٢٨** — **رأسُ السطح الإداريّ صار
		// أربعةَ وسائطَ مشروحة** (`RequireAuth` · قدرةٌ · سياسةٌ ·
		// تأكيدٌ)، **والشرحُ يُحسَب بالأحرف.** **والمعنى لم يتبدّل**:
		// في الرأس لا في الوسط.
		end := at + 2200
		if end > len(src) {
			end = len(src)
		}
