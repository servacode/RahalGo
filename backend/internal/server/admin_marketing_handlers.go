...
func (s *Server) handleListSettings(w http.ResponseWriter, r *http.Request) {
	...
	type setting struct {
		...
		// ══════════════════════════════════════════════════════════
		// **وأيُّ مفتاحٍ يُحرَّر يقوله المحرّكُ لا اللوحة**
		// ══════════════════════════════════════════════════════════
		//
		// **وقدرةُ المفتاح تتبع أثرَه** (`settingCapability`) —
		// **أمنيٌّ أو ماليٌّ أو عامّ.** **واللوحةُ كانت تفتح التحرير
		// باسم `admin`**، **فمن ملك المفتاحَ الماليَّ وحدَه لم يُحرّره،
		// ومن لم يملك الأمنيَّ رأى حقلاً يُردّ ٤٠٣.** (قِيس
		// ٢٠٢٦-٠٩-١٣.)
		//
...
func (s *Server) handleSetSetting(w http.ResponseWriter, r *http.Request) {
	...
	// ══════════════════════════════════════════════════════════════
	// **والقدرةُ تتبع المفتاحَ لا المسار** — `ADG-1` · `AQ-1`
	// ══════════════════════════════════════════════════════════════
	//
	// **«ولكلّ إعدادٍ صلاحيّتُه بحسب أثره»** بنصّ العقد — **فلا
	// يُحرَس المسارُ بقدرةٍ واحدةٍ لمئةِ مفتاح.**
	//
	// **والحارسُ هنا لا في المسار**: **القدرةُ لا تُعرَف إلّا بعد
	// قراءة المفتاح.**
	if need := settingCapability(key); !s.hasCapability(r, need) {
		s.logger.Info("التخويل: مُنع تبديلُ إعداد",
