...
var domainRules = []domainRule{
	...
	// **لا `auth_handlers.go` كلُّه** — **فيه كلُّ أبواب التوثيق،
	// **ومن وسّعها إليه أيقظ `XG-49` عند كلّ مسٍّ للدخول أو
	// التسجيل** — **وإنذارٌ يُطلَق دائماً لا يُقرأ.**
	//
	// **والحذفُ الذاتيُّ ليس بابَ الأدمن**: `AdminUpdateUser` لا
	// تقبل `deleted` — **وذاك حدُّ الملكيّة بين `XG-39` و`XG-49`.**
	{
		Name:     "ACCOUNT_DELETION",
		Match:    []string{"backend/internal/identity/deletion.go"},
		Flows:    []string{"F-36"},
		Apps:     []string{"customer", "driver", "merchant", "rep"},
		Modes:    []Mode{ModeSecurity},
		Packages: []string{"internal/qa", "internal/identity"},
		Why:      "مسٌّ لحذف الحساب ⇒ تجريدٌ وإبطالُ جلسات (`XG-49`)",
	},
	// ── الأحداثُ والإشعار ──────────────────────────────────────────
	{
		Name: "EVENTS",
