...
func (s *Service) UpdateItem(ctx context.Context, actorID, itemID string, in MenuItemInput, ip string) error {
	// ══════════════════════════════════════════════════════════════════
	// **والقسمُ إلزاميٌّ في التعديل كما في الإنشاء**
	// ══════════════════════════════════════════════════════════════════
	//
	// (قرارُ المالك ٢٠٢٦-٠٨-٢٢: «إضافة الصنف القسم إلزامي، لا يجب أن
	//  يكون اختياري فيضيف صنف بدون قسم».)
	//
	// **كان `CreateItem` يفحصه و`UpdateItem` لا يفحصه** — فمن أنشأ صنفاً
	// بقسمٍ ثمّ عدّله بلا قسمٍ **أفلت من الحارس**، وصار صنفاً لا يراه
	// زبون: **المتجرُ يظنّه معروضاً ويدفع عمولةً على ما لا يُباع.**
	//
	// **وحارسٌ على بابٍ واحدٍ من بابين ليس حارساً.**
	//
	// # لكنّ «لم يُرسَل» ليس «انزعه»
	//
	// (شكوى المالك ٢٠٢٦-٠٨-٢٢: «أريد تعديل الصورة يقول اختر القسم».)
	//
	// **وكان الحارسُ يرفض `nil` أيضاً** — وهو خطأ: نافذةُ التعديل في
	// اللوحة **لا ترسل القسمَ أصلاً** لأنّها لا تبدّله، **فصار تبديلُ
	// صورةٍ أو اسمٍ مستحيلاً** على كلّ صنفٍ في المنصّة.
	//
	// **والاستعلامُ أسفلَه يفرّق بينهما أصلاً**: `NULL` يُبقي القسمَ
	// كما هو، **والفراغُ الصريحُ وحدَه ينزعه.** فالحارسُ يحرس النزعَ
	// لا السكوت.
