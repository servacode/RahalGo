...
//
// ══════════════════════════════════════════════════════════════════════
// **وتبديلُ الرقم يُسقط توثيقَ واتساب معه**
// ══════════════════════════════════════════════════════════════════════
//
// (قرارُ المالك ٢٠٢٦-٠٨-١٣ بعد أن قِيس العطب: «نترك العمودين ونصلح
//
//	العيب» · وقاعدتُه من قبل: «ما يصير رقم الهاتف مختلف عن واتساب، هيك
//	تخرب الدنيا».)
//
// # ما كان يقع
//
// **كان يبدّل `phone` ولا يمسّ `whatsapp_phone`.** فمن بدّل رقمَه صار
// له رقمان مختلفان في القاعدة: **الجديدُ للدخول، والقديمُ ممهورٌ
// بختم «موثَّق».**
//
// **وأخطرُ ما فيه أنّه صامت**: لا خطأَ ولا سجلّ — **ورمزُ استعادة رمز
// الأدمن يُرسَل إلى رقمٍ لم يعد لصاحبه.** ومن بدّل رقمَه لأنّه فقد
// القديم **يُرسَل رمزُ فتح لوحته إلى من يحمله الآن.**
//
// # ولماذا يُمسح ولا يُنقَل
//
// **التوثيقُ إثباتٌ لا بيان**: معناه «أثبت هذا الشخصُ أنّ هذا الرقم
// له». **ونقلُ الختم إلى رقمٍ جديدٍ يقول إنّه أُثبت ولم يُثبت** —
// **وختمٌ يكذب أسوأُ من غيابه**، خصوصاً وهو بابُ استعادة لوحة المالك.
//
// **فيُمسح ويُطلب توثيقٌ جديد** — خطوةٌ واحدةٌ يعرفها صاحبُها.
//
// # إلّا أن يكون الجديدُ هو الموثَّق نفسَه
//
// **من وثّق رقماً ثمّ جعله رقمَ دخوله** لم يتبدّل عنده شيء — **وإسقاطُ
// إثباتٍ قائمٍ يُتعب بلا سبب.**
func (r *Repo) SetPhone(ctx context.Context, userID, phone string) error {
	_, err := r.db.Exec(ctx, `
		UPDATE users
		SET phone = $2,
		    whatsapp_phone = CASE WHEN whatsapp_phone = $2
		                          THEN whatsapp_phone ELSE NULL END,
		    whatsapp_verified_at = CASE WHEN whatsapp_phone = $2
		                                THEN whatsapp_verified_at ELSE NULL END,
		    updated_at = now()
...
func (r *Repo) GrantRole(ctx context.Context, userID, role string, grantedBy *string) error {
	// ══════════════════════════════════════════════════════════════════
	// **والحارسُ هنا لا عند كلّ نداء**
	// ══════════════════════════════════════════════════════════════════
	//
	// (قرارُ المالك ٢٠٢٦-٠٨-١٦: «ممنوعٌ منعاً باتاً أن يأخذ أكثرَ من
	//  دور» — تأكيدُ قراره ٢٠٢٦-٠٨-١٠.)
	//
	// **وكان الفحصُ في `AdminGrantRole` وحدَه** — **و`EnsureUserWithRole`
	// تمنح مباشرةً بلا مرورٍ به.** وهي طريقُ فتح المتجر: **يُكتب هاتفُ
	// سائقٍ في خانة صاحب المتجر فيصير سائقاً وصاحبَ متجرٍ معاً** — وهو
	// عينُ التعارض الذي كُتبت القاعدةُ لمنعه.
	//
	// **وازداد الطريقُ استعمالاً** حين صار هاتفُ المالك إلزاميّاً في
	// نموذج المتجر (٢٠٢٦-٠٨-١٥) — **فبابٌ كان ضيّقاً صار الباب.**
	//
	// **فيُنقل الحكمُ إلى المنح نفسِه**: من بلغه فقد وجب عليه، **ولا
	// نداءَ جديدٌ ينساه غدا.**
	if err := r.ensureOnePrimary(ctx, userID, role); err != nil {
		return err
	}
	_, err := r.db.Exec(ctx, `
		INSERT INTO user_roles (user_id, role_code, granted_by) VALUES ($1, $2, $3)
...
func (r *Repo) RevokeRefreshAndDevice(ctx context.Context, tokenHash, deviceToken string) (userID, sessionID string, err error) {
	...
	// ══════════════════════════════════════════════════════════════
	// **ووجهاتُ هذه العائلة تُحذف بمِلكيّتها لا برمزٍ يُقرأ**
	// ══════════════════════════════════════════════════════════════
	//
	// **دورةُ ٦٠**: **كان الحذفُ بالرمز الذي يرسله الجهاز وحدَه** —
	// **ورمزُ `FCM` قد لا يُقرأ ساعةَ الخروج**، **فيمضي الخروجُ
	// ناجحاً وتبقى الوجهةُ حيّةً حتّى يقتلها القِدَم.**
	//
	// **وخروجٌ نجح لا يحتمل «سيُنظَّف لاحقاً».**
	//
	// **والعائلةُ معروفةٌ هنا بلا سؤال** — `RETURNING session_id`.
	//
	// **وبها يُحذف كلُّ ما سجّلته هذه الجلسة**: **رمزٌ دُوّر مرّتين
	// يترك صفّين، ولا يُقرأ من الجهاز إلّا آخرُهما** — **والمِلكيّةُ
	// تطالهما.**
	//
	// **ولا تُحذف وجهةُ عائلةٍ أخرى**: **جلسةٌ ثانيةٌ للحساب نفسِه
	// تبقى**، **ورمزٌ انتقل إلى حسابٍ آخرَ صار لعائلته فلا يُمَسّ.**
	if _, err = tx.Exec(ctx,
		`DELETE FROM device_tokens WHERE session_id = $1::uuid`,
		sessionID); err != nil {
		return "", "", err
	}
	// **ورمزُ الجهاز يبقى مقبولاً لصفوفٍ قديمةٍ بلا مالك** —
	// **سُجّلت قبل هذا العقد فمِلكيّتُها `NULL`.** **ولا يُعتمد
	// عليه**: **إن لم يصل لم يُكسَر شيء.**
	if deviceToken != "" {
		if _, err = tx.Exec(ctx, `
			DELETE FROM device_tokens
			 WHERE token = $1 AND user_id = $2 AND session_id IS NULL`,
