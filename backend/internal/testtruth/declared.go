...
var Gaps = []GapDecl{
	...
	// ══════════════════════════════════════════════════════════════
	// **`XG-48` — وحدةُ الانتقال تمسك معاملةً وتقرأ من المَسبَح**
	// ══════════════════════════════════════════════════════════════
	//
	// # وهو غيرُ `XG-46` وإن كان من عائلته
	//
	// **`XG-46` أُغلقت على نطاقها المُثبَت**: وحدةُ الإنشاء
	// (`CreateTx`). **وهذه وحدةُ الانتقال** (`transitionTx`) —
	// **بابٌ آخرُ وشيفرةٌ أخرى وفروعٌ أخرى.**
	//
	// **والآلةُ التي أصلحتهما واحدة** (`s.on(tx)`) — **والهويّةُ
	// ليست واحدة**، **ولا يُوسَّع عيبٌ مُغلَقٌ ليبتلع ما اكتُشف
	// بعده.**
	//
	// # كيف انكشف
	//
	// **حارسُ الوصلة الواحدة الذي أُضيف في دورةِ ٤٩** لبابِ الانتزاع
	// (`D7` وضع فيه معاملة) — **فأمسك جموداً ليس من `D7`**:
	// **الكومةُ تقول `transitionTx` ← `MerchantsSelfManage` ←
	// `settings.Get` ← `Acquire`.**
	//
	// # ومواضعُه
	//
	//	نمطُ الإدارة        ←  في كلّ انتقال
	//	مهلةُ تدارُك الزبون  ←  في إلغائه وحدَه
	//	نسبةُ تعويض السائق  ←  في التعذّر بذنبِ زبونٍ أو متجر
	//
	// **وعمولةُ المتجر ليست موضعاً**: `orderPct` تردّ مؤشّراً غيرَ
	// عدمٍ دائماً، **فقراءةُ الإعداد لا تُبلَغ من هذا المسار.**
	{ID: "XG-48", Title: "وحدةُ الانتقال تمسك معاملةً وتقرأ الإعداداتِ من المَسبَح",
		Severity: "HIGH",
		Fixed: "**قراءاتُ الوحدة صارت من مُنفِّذها** — `unit := s.on(tx)` " +
			"في مفتتح `transitionTx`، **وهي الآلةُ نفسُها التي أصلحت " +
			"`CreateTx` في دورةِ ٣٩** (`settings.Store.On(q)`): **ولا " +
			"دالّةَ `…Tx` موازيةٌ تُكتب ولا معجمَ إعداداتٍ ثانٍ.** " +
			"**وثلاثةُ مواضعَ لا واحد**: نمطُ الإدارة · ومهلةُ تدارُك " +
			"الزبون · ونسبةُ تعويض السائق (عبر `settle`). " +
			"**وقيس بمَسبَحٍ سقفُه واحدةٌ على المسار الحقيقيّ**: " +
			"**جمد بابُ الانتزاع حتّى انفجرت مهلتُه، وصار يردّ في " +
			"٤٨ جزءاً من الألف** — **ومئةُ جولةٍ بلا جمود.** " +
			"**والفروعُ مُشيت كلُّها**: نمطا الإدارة، وإلغاءُ الزبون، " +
			"والتسليمُ والتسوية، والتعذّرُ والتعويض. " +
			"**وشاهدان سالبان**: أُعيدت قراءةُ نمط الإدارة إلى " +
			"المَسبَح فجمدت الأبوابُ الثلاثة، وأُعيدت `settle` فجمد " +
			"فرعُ التعويض ٣٢ ثانية. " +
			"**ولا خروجَ إلى الشبكة نُقل إلى داخل المعاملة**: البثُّ " +
			"والإشعارُ والتحويلُ التلقائيُّ بعد التثبيت كما كانت " +
			"(`R24`/`XG-44`)."},
	...
	{ID: "XG-49", Title: "الحذفُ الذاتيُّ يُبطل الجلسات ولا حارسَ دائمٌ يُثبته",
		Severity: "HIGH",
		// ══════════════════════════════════════════════════════════
		// **أُغلقت في بوّابة ما قبل الإطلاق** (٢٠٢٦-٠٩-١٣)
		// ══════════════════════════════════════════════════════════
		Fixed: "‏`TestSEC7_SelfDeleteRevokesEverything` — يمشي المسارَ " +
			"الحقيقيَّ (طلبُ رمزٍ ⇒ تأكيدٌ برمزٍ مبصومٍ كما يبصمه المحرّك ⇒ " +
			"تجريد) ويقرأ الثلاثةَ معاً لعائلتين: رمزُ الوصول يُردّ · ولا " +
			"صفَّ تجديدٍ حيّ · ولا وجهةَ دفعٍ باقية · والحالُ `deleted`. " +
			"وكُشف به عطبٌ حقيقيٌّ لم يكن مسجَّلاً: وجهاتُ الدفع كانت تبقى " +
			"بعد الحذف، فأُصلح في الدورة نفسِها. وشاهدٌ سالبٌ: نزعُ قطعِ " +
			"الوجهات يُسقط الحارسَ ويُسمّي الخرق."},
...
var TestMap = map[string]TestDecl{
	...
	"TestXG48_CustomerCancelNeedsOneConnection": {
		Level: L5, Flows: []string{"F-17"},
		Gaps:     []string{"XG-48"},
		Modes:    []string{"FULL", "CONCURRENCY"},
		Evidence: []string{"http"},
	},
	"TestXG48_DeliverySettlementNeedsOneConnection": {
		Level: L5, Flows: []string{"F-13", "F-14"},
		Gaps:     []string{"XG-48"},
		Modes:    []string{"FULL", "CONCURRENCY", "FINANCIAL"},
		Evidence: []string{"http"},
	},
	"TestXG48_FailedDeliveryCompensationNeedsOneConnection": {
		Level: L5, Flows: []string{"F-16"},
		Gaps:     []string{"XG-48"},
		Modes:    []string{"FULL", "CONCURRENCY", "FINANCIAL", "RELEASE"},
		Evidence: []string{"http", "db"},
	},
	"TestXG48_TransitionUnderRepetitionOnOneConnection": {
		Level: L5, Flows: []string{"F-08"},
		Gaps:     []string{"XG-48"},
		Modes:    []string{"FULL", "CONCURRENCY"},
		Evidence: []string{"http"},
	...
	// ── ومِلكيّةُ الوجهة عائلةُ جلسة — دورةُ ٦٠ ──────────────────
	"TestD12_LogoutWithoutDeviceTokenStillRemovesBinding": d12Test(),
	"TestD12_SessionScopedCleanupKeepsOtherSession":       d12Test(),
	"TestD12_TokenRotationWithinSessionIsFullyCleaned":    d12Test(),
	"TestD12_DispatcherSilentAfterTokenlessLogout":        d12Test(),
	"TestD12_StressTokenlessLogout":                       d12Test(),
	"TestD12_StressTwoSessionsAndClaim":                   d12Test(),
	"TestD12_FailureBetweenRevokeAndCleanupIsAtomic":      d12Test(),

	// ── إعادةُ الوصل برمزٍ منتهٍ — `D19` · دورةُ ٥٨ ───────────────
	//
	// **والسلوكُ يُقاس في `mobile/shared` بمقبسٍ حقيقيّ** — **وهذان
	// شقّا الدليل في Go**: **شرطُ المحرّك** (`access_expires_at`)
	// **وبقاءُ دليل الجهاز.**
	"TestD19_ServerDeclaresAccessExpiry": {
		Level: L4, Purpose: PurposeFeature,
		Flows:    []string{"F-04", "F-34"},
		Defects:  []string{"D19"},
		Risks:    []string{"R16"},
		Modes:    []string{"SECURITY", "REALTIME", "FULL", "RELEASE"},
		Evidence: []string{"http"},
	},
	"TestD19_ClientEvidenceIsRegistered": {
		Level: L4, Purpose: PurposeFeature,
		Flows:    []string{"F-04", "F-34"},
		Defects:  []string{"D19"},
		Modes:    []string{"ANDROID", "REALTIME", "FULL", "RELEASE"},
...
var DefectFixed = map[string]string{
	...
	"D12": "**ومِلكيّةُ وجهةِ الدفع عائلةُ جلسةٍ لا رمزٌ يُقرأ من " +
		...
		"**وكان الحذفُ برمز `FCM` الذي يرسله العميل وحدَه**، **وقد " +
		"لا يُقرأ ساعةَ الخروج**: **فيمضي الخروجُ ناجحاً وتبقى " +
		"الوجهةُ حيّةً حتّى يقتلها القِدَم** — **وخروجٌ نجح لا " +
		"يحتمل «سيُنظَّف لاحقاً».** **والخادمُ يعرف العائلةَ في " +
		"الموضعين بلا سؤال**: **`sid` في رمز الوصول عند التسجيل، " +
		"و`RETURNING session_id` عند الخروج.** **فصار الحذفُ " +
		"بالمِلكيّة**: **يطال ما دُوّر من رموزٍ في الجلسة نفسِها " +
		"ولا يعرفه الجهاز**، **ولا يطال عائلةً أخرى للحساب نفسِه، " +
		"ولا رمزاً انتقل إلى حسابٍ آخر.** **وصفوفٌ قديمةٌ بلا " +
		"مالكٍ تبقى فارغةَ المِلكيّة ولا يُختلَق لها مالك** — " +
		"**ورمزُ الجهاز يبقى مقبولاً لها توافقاً لا سلطة.** " +
		"**وحقنُ فشلٍ على الحذف يُسقط المعاملةَ كلَّها**: **لا " +
		"جلسةٌ أُبطلت ووجهةٌ باقية.** **والمقيس**: **خروجٌ بلا " +
		"رمزِ جهازٍ ⇒ صفرُ وجهات · دورانٌ×١٠٠ · جلستان×١٠٠ · " +
		"مطالبةُ جلسةٍ جديدة×١٠٠.** " +
		"**وبابُ الخروج صار يُنهي وجهةَ الدفع مع الجلسة في معاملةٍ " +
	...
	"D19": "**حلقةُ إعادة الوصل صارت تطلب رمزاً من سلطته لا تقرأ " +
		...
		"`session.accessToken()` عند كلّ محاولةٍ ولا تجدّد أبداً**: " +
		"**فرمزُ الوصول يعيش ربعَ ساعة**، **ومن عاد من نفقٍ أو من " +
		"الخلفيّة بعدها تُعاد مصافحتُه بالرمز المنتهي نفسِه أبداً** — " +
		"**وعائلةُ التجديد سليمةٌ طوالَ ذلك**، **ولا مخرجَ إلّا خروجٌ " +
		"ودخولٌ بيده.** " +
		"**والسلطةُ هي المنتهى لا الاستثناء**: **المحرّكُ يرسل " +
		"`access_expires_at` وكان التطبيقُ يرميه** — **وقيس أنّ " +
		"مصافحةً فاشلةً لا تحمل رقمَها في حقل**: **٤٠١ ترمي " +
		"`WebSocketException` و٤٠٣ و٥٠٣ ترميان `ProtocolException`**، " +
		"**والرقمُ في نصّ الرسالة** — **فقرارُ أمنٍ على مطابقة نصٍّ " +
		"يتبدّل بتبديل محرّك الشبكة.** " +
		"**ولا تجديدَ عند كلّ انقطاع**: **النفقُ وتبديلُ الشبكة " +
		"وإقلاعُ الخادم ليست انتهاءً** — **ولا يُجدَّد إلّا إذا قال " +
		"المنتهى إنّه انتهى**، **فالمحظورُ والمُبطَلُ رمزُهما لم ينتهِ " +
		"فلا يطرقان بابَ التجديد أصلاً.** " +
		"**والتجديدُ من مالكه لا نسخةٌ عنه** (`ApiClient.refresh`) — " +
		"**ولا نداءَ `/auth/refresh` ثانٍ يُكتب للبثّ.** " +
		"**وقياسُ التزامن كشف عطباً ثانياً**: **سؤالُ المنتهى كان خارجَ " +
		"القفل، فاثنتان وعشرون جولةً من مئةٍ دوّرت رمزَ التجديد " +
		"مرّتين** — **فصار يُسأل ثانيةً تحت القفل.** " +
		"**والمقيسُ بمقبسٍ حقيقيٍّ ومصافحةٍ حقيقيّة**: **المصافحةُ " +
		"الأولى تحمل الجديدَ وحدَه · وتجديدٌ واحدٌ · وحدثٌ يصل · " +
		"وانقطاعُ نقلٍ بصفرِ تجديدات · و٥٠٣ لا تمحو اعتماداً · ووصلةٌ " +
		"واحدة.** **وتكرارٌ: تعافٍ×١٠٠ · وصلٌ عاديٌّ×١٠٠ · تزامنٌ×١٠٠ " +
		"· ٥٠٣×٥٠ · مرفوضٌ×٥٠.** " +
		"**وقبولُ الجهاز باقٍ** (`AND-31`) — **والمحلّيُّ لا يُغنيه.**",
