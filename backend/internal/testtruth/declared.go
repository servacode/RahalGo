...
var Gaps = []GapDecl{
	...
	// ══════════════════════════════════════════════════════════════
	// **`XG-41` — حرّاسٌ ساقطةٌ مضت دورةً بلا أن تُرى**
	// ══════════════════════════════════════════════════════════════
	//
	// # كيف انكشفت
	//
	// **تقريرُ دورةِ ٢٥ قال «صفرُ سقطات» وكان في السجلّ تسعٌ.**
	// **قُرئ السجلُّ وهو يُكتب** — حلقةُ انتظارٍ خرجت حين رأت ثلاثين
	// سطرَ حزمةٍ، **والحزمُ لم تكن قد تمّت.**
	//
	// **والدرسُ ليس في الحلقة**: **رقمٌ يُقرأ من ملفٍّ حيٍّ ليس قياساً**
	// — **ولا يُقرأ إلّا بعد أن يُعلن المنفِّذُ انتهاءه.**
	//
	// # وثلاثٌ منها قائمةٌ اليوم
	//
	// **١ · `admin.setting_update` يُدقَّق بأفضل جهدٍ لغير الحسّاس.**
	// **وذاك عقدُ دورةِ ٢١ نفسُه** (الحسّاسُ في معاملةٍ وما سواه لا) —
	// **لكنّ حارسَ `AQ-4` يقرأ الفعلَ كلَّه صنفاً واحداً.** **فأحدُهما
	// خطأ**: **إمّا يُصنَّف الفعلُ بمفتاحه في الحارس، أو تصير كلُّ
	// كتابةِ إعدادٍ معامليّة.** **ولا يُحسَم بغير قرار.**
	//
	// **٢ · `errors.auth_unavailable` بلا رسالةٍ عربيّة** — **يُقرأ
	// «حدث خطأٌ غير متوقّع».** وُلد في دورةِ ١٦ مع `R16`.
	//
	// **٣ · `PF09_N1` و`PF09_N2N3N8` تسقطان في الحزمة الكاملة وتنجحان
	// وحدَهما** — **وذاك تداخلُ حالةٍ بين فحوصٍ لا عطبُ منتج.**
	//
	// # ولم تُصلَح هنا
	//
	// **دورةُ ٢٦ نطاقُها «أقلُّ صلاحيّة» بنصّ المالك** — **ولا
	// إصلاحاتٍ عارضة.** **فسُجّلت وتُعرَض.**
	{ID: "XG-41", Title: "حرّاسٌ ساقطةٌ مضت دورةً كاملةً بلا أن تُرى",
		Severity: "HIGH"},
