...
var Gaps = []GapDecl{
	...
	// ══════════════════════════════════════════════════════════════
	//
	// **رُصد مرّتين** (دورةُ ٩ ودورةُ ١٠) — **ولا يقع إلّا في الحزمة
	// الكاملة**، وكلُّ فحصٍ يمرّ منفرداً.
	//
	// **والأرقامُ المقيسة**: `TestALT_001` سقط بـ`500` بعد ٣٢ ثانية ·
	// و`TestFIN_CommissionSourceMatrix` بعد ٦٢ و٣٠ ثانية —
	// **ومهلةُ الطلب ثلاثون**، فالانتظارُ تجاوزها.
	//
	// **ولا تشخيصَ مُثبَتاً**: **الظنُّ أنّ معاملاتٍ صارت تُمسك أطولَ
	// منذ أن صار منسّقُ منع التكرار يملكها عبر المعالج كلِّه** —
	// **وظنٌّ ليس قياساً.** **ولم أُثبته فلا أدّعيه.**
	//
	// **وشدّتُها `HIGH` لا أعلى**: **لم تظهر إلّا تحت حمل حزمةٍ
	// تُشغّل ثمانمئةَ فحصٍ بالتوالي**، ولا دليلَ على وقوعها في خدمةٍ
	// حيّة. **ورفعُها إلى مانعةٍ بلا دليلٍ تضخيمٌ، وإهمالُها تسترٌ.**
	//
	// **وما يلزم لحسمها**: قياسُ الأقفال من `pg_locks` أثناء التشغيل
	// الكامل، وقياسُ مدّةِ إمساكِ المعاملة قبل المنسّق وبعده.
