...
func buildFromSteps(steps []osrmStep) *Route {
	...
	// لا بالبحث.
	idx := 0
	// **ودوّارٌ سبق** — انظر طيَّ «الخروج» أدناه.
	lastWasRoundabout := false
	for _, st := range steps {
		m := Maneuver{
		...
		// ══════════════════════════════════════════════════════════════
		// **ودوّارٌ واحدٌ جملةٌ واحدة**
		// ══════════════════════════════════════════════════════════════
		//
		// **(قِيس 2026-09-02 على مسارٍ في الرقّة طولُه أربعةَ عشرَ
		//  كيلومتراً: سبعُ مناوراتِ دوّارٍ لثلاثة دوّارات.)**
		//
		// **وOSRM يرسل مناورتين لكلّ دوّار**: `rotary` عند الدخول
		// و`exit rotary` عند الخروج. **فيسمع السائقُ جملتين لفعلٍ
		// واحد**: «عند الدوّار، خذ المخرج الثاني»، ثمّ وهو داخلَه
		// «اخرج من الدوّار».
		//
		// **والثانيةُ لا تضيف شيئاً** — الأولى قالت المخرجَ برقمه،
		// **والخروجُ هو ما أمر به.** وغوغل يقول واحدةً.
		//
		// **وأسوأُ من التكرار أنّ الرقمين اختلفا**: قِيس دوّارٌ قالت
		// أولاه «المخرج الثاني» وقالت ثانيتُه `exit=1`. **فرقمُ
		// الخروج ليس رقمَ المخرج نفسَه** — هو ترتيبُه في خطوةٍ أخرى.
		//
		// **وتبقى وحدَها إن جاءت وحدَها** — مسارٌ يبدأ داخلَ دوّارٍ
		// لا مناورةَ دخولٍ له، **ومن حذفها مطلقاً أسكت جملتَه
		// الوحيدة.**
		if m.Kind == KindExitRoundabout && lastWasRoundabout {
			lastWasRoundabout = false
		} else {
			lastWasRoundabout = m.Kind == KindRoundabout
