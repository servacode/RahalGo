...
export function NotificationBell({
  ...
  /* ══════════════════════════════════════════════════════════════════
     **والعدّادُ بجانب الجرس — لا فوقه**
     ══════════════════════════════════════════════════════════════════

     (شهده المالك ٢٠٢٦-٠٨-١١ ثلاثَ مرّات، وآخرُها دلّني على السبب:
      «المشكلةُ تحدث عند وجود رقمٍ بعدّاد الجرس — احذف الرقمَ وتأكّد».)

     **وهو محقّ: الجرسُ سليمٌ وحدَه، ويُقصّ حين يظهر العدد.** والعلّةُ
     ثلاثةٌ مجتمعة، وكلُّها من إجلاسِ العدّاد فوق الأيقونة:

     **الأولى** أنّ الفقّاعةَ تغطّي ربعَ الجرس فتأكل حافّتَه العليا.
     **والثانية** أنّ حلقتَها (`ring-2 ring-surface`) تُطلى بلون السطح
     **فتقطع خطَّ الجرس قطعاً نظيفاً** — وهو «البوردر» الذي رآه.
     **والثالثة** أنّ ما يخرج علوّاً يقع تحت مِقصِّ `overflow` في صفّ
     الأدوات — **وحشوةٌ علويّةٌ عالجت الثالثةَ ولم تمسّ الأُوليَين.**

     **فالحلُّ ألّا تُجلس فوقه أصلاً**: رقمٌ يتلو الجرسَ في سطره،
     **فلا تغطيةٌ ولا حلقةٌ تقطع ولا شيءَ يخرج من الصندوق.**

     **ويكبر الزرُّ قليلاً في العرض** — وهو ثمنٌ زهيدٌ مقابل جرسٍ يُرى
     كاملاً، **والعددُ نفسُه صار أوضحَ إذ لم يعد فوق خطوط.** */
  const cls =
    "flex items-center gap-1 rounded-control p-1.5 text-ink-muted transition-colors hover:text-accent-text";
  const inner = (
    <>
      ...
      <IconBell size={19} />
      {unread > 0 && <CountBadge count={unread} max={9} />}
    </>
  );
