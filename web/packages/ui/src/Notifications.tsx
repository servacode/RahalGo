...
export function NotificationBell({
  ...
  const cls =
    "relative flex items-center rounded-control p-1.5 text-ink-muted transition-colors hover:text-accent-text";
  const inner = (
    <>
      ...
      <IconBell size={19} />
      {unread > 0 && (
        <span className="absolute -top-0.5 -end-0.5 flex h-4 min-w-4 items-center justify-center rounded-badge bg-accent px-1 text-2xs font-bold text-on-accent">
          {unread > 9 ? "9+" : unread}
        </span>
      )}
    </>
