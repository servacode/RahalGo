...
export interface ChipDef<K extends string = string> {
  ...
  label: ReactNode;
  /**
   * **عددٌ بجانب الاسم — تُرسمه الحبّةُ لا صاحبُها.**
   *
   * (شهده المالك ٢٠٢٦-٠٨-١١ بصورتين متجاورتين: عددٌ في فقّاعةٍ هنا،
   *  ورقمٌ عارٍ ملتصقٌ بالاسم هناك — «مشكلةُ الأرقام».)
   *
   * **وكان يُمرَّر داخلَ `label`** — نصّاً بـ`opacity-70` بجانب الاسم.
   * **ورقمٌ عارٍ يُقرأ جزءاً من الاسم**: «تمّ التسليم ١» تبدو حالةً
   * اسمُها كذلك، لا حالةً فيها طلبٌ واحد.
   *
   * **والحبّةُ وحدَها تعرف أمختارةٌ هي أم لا** — ومن يبني الاسمَ من
   * خارجها لا يملك أن يلوّن العدّادَ بحسب ذلك، **فيبقى بلونٍ واحدٍ على
   * أرضيّتين متعاكستين.**
   */
  count?: number;
    ...
    >
      {items.map((c) => {
        ...
        return (
          ...
          >
            {c.label}
            {c.count != null && <CountBadge count={c.count} on={sel} />}
          </button>
        );
      })}
    </div>
