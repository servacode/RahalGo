...
export const PANEL_PATHS = {
  admin: "/dashboard",
  // **ولوحةُ المتجر حُذفت من الويب ٢٠٢٦-٠٨-٢٦** — ملفّاتُها معها.
  // **وتطبيقُ المتجر يحمل كلَّ ما كانت تحمل.**
  customer: "/",
  /**
   * **بابُ التطبيقات — وجهةُ كلّ من ليس إدارة.**
   *
   * (قرارُ المالك ٢٠٢٦-٠٨-٢٦: «الويب صار للإدارة وتصفّح الموقع فقط».)
   */
  app: "/app",
} as const;
...
export function homeFor(roles: string[]): Destination {
  ...
  // ══════════════════════════════════════════════════════════════════
  //
  // (قرارُ المالك ٢٠٢٦-٠٨-٢٦: «كلّ شخص عنده تطبيقه، والويب صار للإدارة
  //  وتصفّح الموقع فقط».)
  //
  // **وكان المتجرُ يُساق إلى `‎/store`** — لوحةٌ كاملةٌ تُصان مرّتين:
  // مرّةً في الويب ومرّةً في التطبيق، **فتفترقان بلا أن يصرخ شيء.**
  //
  // **ومن دخل بدورٍ ليس دورَ إدارةٍ يُساق إلى باب التطبيقات** — لا إلى
  // صفحةٍ فارغةٍ ولا إلى أربعمئةٍ وأربعة.
  return { origin: "", path: PANEL_PATHS.app };
}
