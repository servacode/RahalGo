  ...
  async function onVerify(e: React.FormEvent) {
    ...
    try {
      ...
      if (!result.user.roles.includes("merchant")) {
        setError(m.merchant.notAllowed);
        setBusy(false);
        return;
      }
    ...
    <main className="flex min-h-screen items-center justify-center bg-page p-4">
      <div className="w-full max-w-sm rounded-card border border-line bg-surface p-8 shadow-sm">
        <div className="mb-6 text-center">
          ...
          <h1 className="text-xl font-bold">{m.merchant.loginTitle}</h1>
          <p className="mt-1 text-sm text-ink-muted">{m.merchant.loginSubtitle}</p>
        </div>
