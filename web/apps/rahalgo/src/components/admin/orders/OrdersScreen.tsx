...
const OPS_NEXT: Record<string, string[]> = {
  ...
  assigned: ["at_pickup"],
  // ══════════════════════════════════════════════════════════════════
  // **و«فشل التوصيل» للمكتب بعد التسليم إلى السائق**
  // ══════════════════════════════════════════════════════════════════
  //
  // **(قِيس ٢٠٢٦-٠٩-٠٢: المحرّكُ يعطي المكتبَ حقَّ الإفشال من أربع
  // حالاتٍ — والشاشةُ تُخفيه في أربعتها.)**
  //
  // **والحاجةُ إليه واقعةٌ لا مفترضة** (بلاغُ المالك ٢٠٢٦-٠٩-٠٢):
  // **هاتفُ السائق يموت في منتصف الطريق فيقف الطلبُ معلّقاً**، ولا
  // يملك المكتبُ أن يُنهيه.
  //
  // # وليس هذا إلغاءً
  //
  // **والفرقُ ليس لفظاً**: الإلغاءُ يقول «رفضنا أن نخدم»، **والفشلُ
  // يقول «حاولنا فلم ننجح»** — سائقٌ انقطع، أو زبونٌ لا يردّ، أو
  // عنوانٌ لا يوجد.
  //
  // **وقرارُ المالك ٢٠٢٦-٠٨-٠٤ يمنع الأوّل** («وافقنا على طلب…
  // ونحن ما فتحنا المنصة مشان نرفض طلبات») **ولا يمسّ الثاني** —
  // فمن التزم بطلبٍ لم يلتزم بأن ينجح فيه مهما وقع.
  //
  // **والسببُ إلزاميٌّ** (`requiresReason` في المحرّك)، والشاشةُ
  // تفتح نافذتَه من نفسها لأنّ «failed» في `DESTRUCTIVE`.
  //
...
const DRIVER_ONLY = new Set([
  ...
  "at_dropoff",
  "delivered",
  // **و«فشل التوصيل» خرجت من هنا** ٢٠٢٦-٠٩-٠٢ — انظر `OPS_NEXT`.
  // **وكانت تُخفيه عن المكتب في الحالات الأربع التي يسمح بها
  // المحرّك**، فيبقى طلبٌ انقطع سائقُه معلّقاً بلا مُنهٍ.
]);
          ...
          o.ops_stages?.length ? (
            <OrderTrackV
              stages={o.ops_stages.map((id, i) => ({
                ...
                // والخطُّ فوق مرحلة الإسناد هو لحظتُها بعينها،
                // **فالاسمُ عليه يجيب «من أنهى الانتظار».**
                //
      ...
      {data && (
        /* ══════════════════════════════════════════════════════════
           **ولا سطرَ إجماليٍّ تحت القائمة**
           ...
           (قرارُ المالك ٢٠٢٦-٠٨-١٣: «هي ألغِها من الطلبات وسجلّ
            الطلبات».)

           **والسجلُّ يقوله فوقُ في بطاقة «كل الحالات»** — رقمٌ واحدٌ
           في موضعين يُقرأ رقمين: **أهذا كلُّ السجلّ أم كلُّ ما
           رُشِّح؟**

           **وشاشةُ العمل لا تُسأل «كم»** بل «أيُّها ينتظر الآن» —
           والترقيمُ نفسُه يقول كم صفحةً بقيت لمن أراد. */
...
function OrderActions({
  ...
  // ══════════════════════════════════════════════════════════════════
  //
  // (تدقيقُ لوحة الإدارة ٢٠٢٦-٠٨-٢٣، وقرارُ المالك: «أكمل الباقي».)
  //
  // **`POST /admin/orders/{id}/recompute` قائمٌ في المحرّك للأدمن
  // وحدَه** — **ولا شيءَ في اللوحة ينادِيه.** فمن أراد إصلاحَ تسويةٍ
  // قديمةٍ احتاج من يفتح له طرفيّة.
  //
  // **ويُصلح بقيدٍ مقابلٍ لا بتصفير البيانات** — فالدفترُ يبقى قابلاً
  // للمراجعة: **تسويةٌ تُمحى تجعل مجموعَ الأمس لا يطابق مجموعَ اليوم
  // ولا أحدَ يعرف لماذا.**
  //
  // **ولا يُعرض إلّا على طلبٍ أُغلق** — **وإعادةُ حسابِ ما لم ينتهِ
  // تكتب قيداً ثمّ يتبدّل الأصلُ بعده.**
  ...
  return (
    <>
      ...
      {/* ══════════════════════════════════════════════════════════════
        * **وتسويةُ البضاعة قيدٌ ماليّ** (٢٠٢٦-٠٩-١٣)
        * ══════════════════════════════════════════════════════════════
        *
        * **و`POST /orders/{id}/settle-goods` بـ`finance.manage`** —
        * **وقِيس: موظّفُ العمليّات كان يرى «إلى المكتب — المنصة
        * تتحمل» وكلُّ ضغطةٍ تُردّ ٤٠٣.**
        *
