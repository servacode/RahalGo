...
/**
 * serverApiBase **عنوانُ المحرّك للجلبِ من داخل الحاوية.**
 *
 * ══════════════════════════════════════════════════════════════════════
 * **عطبٌ كشفه مركزُ التنزيل** (`DLC`، ٢٠٢٦-٠٩-١٣)
 * ══════════════════════════════════════════════════════════════════════
 *
 * **و`apiUrl` عنوانٌ عامٌّ يراه المتصفّح** — **وحاويةُ الويب لا تصل
 * إليه بالضرورة**: **قِيس على التجهيز أنّ `http://<ip>:8080` من داخل
 * حاوية الويب ينتهي بمهلة** (ارتدادُ العنوان إلى المضيف محجوب).
 *
 * **وأثرُه كان صامتاً**: **خريطةُ الموقع تُولَّد بلا أقسامٍ** — **١٤
 * رابطاً ساكناً ولا قسمَ واحد** — **ولا سطرَ خطأٍ في مكان.** (قِيس
 * ٢٠٢٦-٠٩-١٣، وهو سابقٌ لمركز التنزيل.)
 *
 * **والعنوانُ الداخليُّ يعمل**: `http://api:8080` في شبكة التركيب.
 *
 * **ويُقرأ من البيئة ويرتدّ إلى العامّ** — **فبيئةٌ لم تُضبَط تعمل كما
 * كانت ولا تنكسر**، **وبيئةٌ ضُبطت تُصلَح.**
 *
 * **ولا يصل المتصفّحَ أبداً**: **ليس في `PublicConfig`** — **وعنوانٌ
 * داخليٌّ في صفحةٍ رابطٌ لا يُفتح.**
 */
export function serverApiBase(): string {
  const inner = (process.env.RAHALGO_API_INTERNAL_URL ?? "").trim();
  return inner || readServerConfig().apiUrl;
}
