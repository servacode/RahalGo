...
// والشكاوى والطوارئُ والإعدادات — **لأنَّ بنداً بلا `roles` كان يظهر
// للكلّ.** (قِيس في متصفّحٍ على التجهيز ٢٠٢٦-٠٩-١٢، وشرطُ المالك
// في بندِ ٩ ألّا يراها.)
//
// **فالبندُ بلا بابِ قدرةٍ يُحجَب عمّن لا يحمل دورَ لوحةٍ قديماً.**
{
  const lay = layout.replace(/\s+/g, " ");
  if (!lay.includes("const legacyPanel = PANEL_ROLES.some(has)")) {
    problems.push(
      "**الترشيحُ لا يفرّق صاحبَ القدرةِ من صاحب اللوحة** — " +
        "**فيرى بنداً لا يفتحه** (بندُ ٩)",
    );
  }
  if (!lay.includes("if (i.caps) return i.caps.some(can); if (!legacyPanel) return false;")) {
    problems.push(
      "**بندٌ بلا بابِ قدرةٍ يظهر لمن لا يحمل دورَ لوحة** — " +
        "**ورؤيةُ بابٍ لا يُفتح أسوأُ من عدم رؤيته** (`R-34`)",
    );
  }
  // **ومن هبط على «الرئيسيّة» بلا `admin.stats` رأى عطباً** — فيُنزَل
  // على أوّل بابٍ يملكه.
  ...
  if (problems.length === 0) notes.push("صاحبُ القدرةِ يرى بابَه وحدَه — ويهبط عليه");
}
