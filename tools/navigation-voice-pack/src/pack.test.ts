...
test('والنصُّ المرئيُّ بلا تشكيلٍ كامل — والشدّةُ تبقى', () => {
  ...
  assert.ok(arrived);
  assert.equal(arrived.displayText, 'لقد وصلت إلى وجهتك.');
});
...
test('وجملةُ الدوّار بالمخرج المطلوب', () => {
  ...
  assert.ok(r);
  assert.equal(r.ttsText, 'عِندَ الدَّوّار، اِسلُك المَخرَج الثّاني.');
});
...
test('وجدولُ المراجعة يهرّب الفاصلة', () => {
  ...
  assert.ok(csv.includes('"عِندَ الدَّوّار، اِسلُك المَخرَج الثّاني."'));
});
