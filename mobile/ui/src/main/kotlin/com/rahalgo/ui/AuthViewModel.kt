...
class AuthViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun logout() {
        ...
        backend.session.clear()
        user = null
        // **وحدُّ الجلسة يُطبَّق مع الخروج** — `CUST-DEF-004`: تُمحى السلّةُ
        // وتُوقَف الوصلةُ الحيّةُ وتُنعَش الشاشاتُ (يسجّلها التطبيقُ في
        // `AppCore.install`)، **فلا يرث الحسابُ التالي سلّةَ من قبله ولا
        // وصلتَه.** ويقع قبل نداء الشبكة فلا يتأخّر بتعذّرها.
        AppCore.afterLogout()
