...
private fun Signed(theme: ThemeState, dark: Boolean) {
    ...
    // ══════════════════════════════════════════════════════════════════
    // **ويُسجَّل الجهازُ للدفع بعد الدخول** — انظر `ui/Push.kt`.
    // ══════════════════════════════════════════════════════════════════
    //
    // (سؤالُ المالك ٢٠٢٦-٠٨-١٩: «المفروض يصل إشعارٌ لو التطبيق مغلق
    //  صحيح؟» — وكان الجوابُ حينها: لا.)
    //
    // **ومفتاحُه معرّفُ الحساب لا رايةُ «داخل»** — **ومن بدّل حسابَه
    // على الجهاز نفسِه** يُسجَّل من جديد، **وإلّا بقيت أخبارُ الأوّل
    // تصل الثاني.**
    com.rahalgo.ui.RegisterPush(userId = vm.user?.id)
...
private fun SignedIn(
    ...
    // ══════════════════════════════════════════════════════════════════
    //
    // **وكان الزرُّ ينادي ويمضي** — **فمن رُفض إذنُه أو أطفأ خدمتَه
    // ضغطه فلم يقع شيءٌ ولا رسالة**، **ثمّ ضغطه خمساً.**
    ...
    val mapPicker: PointPicker = { onPick, onCancel ->
        PickPoint(
            ...
            onLocate = {
                if (Here.granted(ctx)) Here.refresh(ctx, locating)
            ...
            // ══════════════════════════════════════════════════════
            //
            // **فالزرُّ يدور ما دام النداءُ قائماً** — **وجوابٌ في
            // اللحظة**، **والتعذّرُ يُقال بسببه.**
            locating = locating,
        )
    }
                ...
                if (!guest) {
                    ...
                    androidx.compose.runtime.LaunchedEffect(waiting) {
                        when (waiting.type) {
                            ...
                            // ══════════════════════════════════════════
                            // **وخبرُ حالةِ الطلب يفتح «طلباتي»** (`CAF-13`)
                            // ══════════════════════════════════════════
                            //
                            // **وكان يسقط إلى البيت** — فيضغط الزبونُ «طلبك
                            // في الطريق» فيُفتَح السوقُ لا طلبُه. **والوجهةُ
                            // تبويبُ الطلبات** (لا صفحةَ طلبٍ مفردةٍ في هذا
                            // التطبيق): **طلبٌ لا يملكه لا يظهر — احتياطٌ آمن.**
                            ...
                            // ══════════════════════════════════════════
                            // **ولا وجهةَ متجرٍ أصلاً** (قرارُ المالك
                            // ٢٠٢٦-٠٩-١٥)
                            // ══════════════════════════════════════════
                            //
                            // **ورُفعت من المحرّك ومن اللوحة ومن
                            // التنقية** — **فخبرٌ قديمٌ يحملها يسقط إلى
                            // البيت كأيّ مجهول** (`MD-08`)، **ولا فرعَ
                            // هنا يحرسها.**
