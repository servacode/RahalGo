...
fun CartScreen(
    ...
    Screen {
        ...
        // ══════════════════════════════════════════════════════════════
        // **ولا يُقال «فشل» لما لا يُدرى — ويُقال «لا ندري» صريحاً** (`PC-8`)
        // ══════════════════════════════════════════════════════════════
        //
        // **انقطاعُ الشبكةِ في منتصف الإرسال ليس حسماً**: قد يكون الطلبُ
        // قُيِّد وضاع الجواب. **وكان `uncertain` يُضبط ولا يُعرض** — فيرى
        // صاحبُه رسالةَ خطأٍ عامّةً فيعيد الضغطَ **فيصير طلبُه طلبين.**
        //
        // **فيُقال له صراحةً «لا ندري» ويُوجَّه إلى «طلباتي» قبل إعادة
        // الإرسال** — فلا يُنشئ ثانياً ما قد نجح أوّلاً.
        if (vm.uncertain) {
            Spacer(Modifier.height(8.dp))
        ...
        RahalButton(
            ...
            // ══════════════════════════════════════════════════════════
            // **ومقطوعُ الشبكةِ لا يُرسِل** (`P8-L1-019` §7 · `CUST-12-021`)
            // ══════════════════════════════════════════════════════════
            //
            // **زرٌّ يُضغط بلا شبكةٍ يُطلق نداءً يعلّق ثمّ يفشل** — **فيُقرأ
            // «نجاحاً كاذباً» أو «نقرةً صامتة».** **والحالُ المركزيّةُ
            // `Net.online` تحكم** (وشريطُ الانقطاع فوقَه يقول السبب) — فلا
            // يُرسَل إلّا على شبكةٍ موثَّقة.
            enabled = !vm.busy && address != null && Serving.available &&
                com.rahalgo.ui.Net.online &&
