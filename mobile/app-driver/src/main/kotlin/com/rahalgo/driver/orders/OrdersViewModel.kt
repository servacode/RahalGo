...
class OrdersViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private suspend fun load() {
        ...
        loadChatBadge()
        // ══════════════════════════════════════════════════════════════
        // **والدورةُ نفسُها في `OrdersLoader` — لتُختبر**
        // ══════════════════════════════════════════════════════════════
        //
        // (منظومةُ الاختبار، الطبقةُ الأولى — ٢٠٢٦-٠٨-٢٣.)
        //
        // **وكان ترتيبُ نداءاتها هنا** — فلا يُختبر بلا شبكةٍ ولا
        // جهاز، **وهو الذي انكسر يومَ سبق نداءُ المسار وصولَ
        // الطلبات.**
        //
    ...
    /**
     * **وصلةُ الدورة بالمحرّك** — الوجهُ الوحيدُ الذي يُزيَّف في الاختبار.
     *
    ...
    // ══════════════════════════════════════════════════════════════════
    // **والصوتُ مكتومٌ حتّى يُوجَد صوتٌ يُرضي**
    // ══════════════════════════════════════════════════════════════════
    //
    // (قرارُ المالك ٢٠٢٦-٠٨-٢٣: «ألغِ الصوت، ما بدّنا صوت، انسَ
    //  موضوعَه — واحذف الصوتَ والصوتيّات أيضاً. لاحقاً إذا وجدنا
    //  صوتاً مناسباً أضفناه، وإن لم نجد نلغي الصوتَ بالكامل».)
    //
    // # وما جُرّب قبل هذا القرار
    //
    // **ثلاثةُ أصواتٍ قِيست على جهازه**: مصريٌّ مضغوط (`arz-local`)،
    // ثمّ شاميٌّ عصبيٌّ شبكيّ (`arc-network`)، ثمّ مقاطعُ Piper
    // العصبيّةُ الأردنيّة. **ولم يرضَ عن واحدٍ منها.**
    //
    // **وطلب صوتَ غوغل ماب بعينه** — وهو ملكُ غوغل لا يُشحن، **وصوتُ
    // إنسانٍ بعينه لا يُستنسخ بلا إذنه.**
    //
    // # ولماذا كتمٌ لا حذفٌ للشيفرة
    //
    // **قال «لاحقاً إذا وجدنا صوتاً مناسباً أضفناه»** — فالطريقُ يبقى
    // مفتوحاً بسطرٍ واحدٍ يُقلب. **وحذفُ محرّكِ نطقٍ كاملٍ ثمّ إعادةُ
    // بنائه بعد أسبوعٍ عملٌ يُعاد بلا سبب.**
    //
    ...
    fun toggleVoice() {
        // **ولا يُفتح ما أُغلق بقرار** — الزرُّ يبقى ولا يعمل حتّى
        // يُقلب `voiceEnabled`.
