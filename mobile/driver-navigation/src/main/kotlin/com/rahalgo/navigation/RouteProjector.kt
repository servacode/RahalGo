...
object RouteProjector {
    ...
    /**
     * **أيمينَ الخطّ أم يسارَه** — يردّ `+1` يساراً و`-1` يميناً.
     *
     * **والحسابُ في مستوٍ محلّيّ**: خطُّ العرض متساوٍ في كلّ مكان،
     * وخطُّ الطول يُضرَب بجيبِ تمامِ العرض — **فالإشارةُ صحيحةٌ
     * والمسافةُ تُؤخذ من [Hit.offRouteM] لا منها.**
     *
     * **وقطعةٌ صفريّةُ الطول تردّ صفراً** — لا جهةَ لنقطة.
     */
    fun sideOf(lat: Double, lng: Double, a: GeoPoint, b: GeoPoint): Double {
        val klat = 111320.0
        val klng = 111320.0 * cos(Math.toRadians(lat))
        val vx = (b.lng - a.lng) * klng
        val vy = (b.lat - a.lat) * klat
        val wx = (lng - a.lng) * klng
        val wy = (lat - a.lat) * klat
    ...
    data class Hit(
        ...
        /**
         * ══════════════════════════════════════════════════════════
         * **الإزاحةُ الموقّعة — المرحلة ٨ب**
         * ══════════════════════════════════════════════════════════
         *
         * (قرارُ المالك ٢٠٢٦-٠٨-٢١، البند ٨.)
         *
         * **موجبٌ يعني يسارَ اتّجاه السير، وسالبٌ يمينَه.**
         *
         * **و[offRouteM] تقول «كم يبعد» ولا تقول «من أيّ جهة»** —
         * والجهةُ هي ما يميّز طريقاً موازياً عن تذبذبٍ حولَ الخطّ:
         * **من يسير على الموازي يبقى في جهةٍ واحدة، ومن يسير على
         * المسار يتذبذب حولَه.**
         *
         * **ولا يُغيّر حكمَ الإسقاط**: `offRouteM` كما هي،
         * **والاختيارُ بينَ القطع كما كان.** هذا حقلٌ يُضاف لا
         * منطقٌ يُبدَّل.
         */
        ...
        for (i in from..max(from, to)) {
            ...
            if (score < bestScore) {
                ...
                best = Hit(
                    ...
                    offRouteM = dist,
                    segment = i,
                    // **والجهةُ بالضربِ الاتّجاهيّ** — إشارةُ
                    // `(b−a) × (p−a)` في مستوٍ محلّيّ.
                    lateralSignedM = dist * sideOf(lat, lng, a, b),
                )
            }
        }
        return best
