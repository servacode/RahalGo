...
fun TripMap(
    ...
    // ══════════════════════════════════════════════════════════════════
    // **وأساسُ التقدّم يُصفَّر مع كلّ خطٍّ جديد**
    // ══════════════════════════════════════════════════════════════════
    //
    // **(بلاغُ المالك ٢٠٢٦-٠٩-٠٢:** «يختفي الخطُّ الأزرقُ الذي يدلّ
    // على الطريق في منتصف الرحلة» — **«وهذه ألعنُ من الانهيار».)**
    //
    // **والخطُّ المرسومُ يُعاد جلبُه من موضع السائق كلّما تحرّك** —
    // فيقصر: `route` الجديدُ يبدأ من حيث هو، **وما مشاه ليس فيه.**
    //
    // **و`progressM` يُقاس على مسار المحرّك الذي بدأ منه** — فيطول.
    //
    // **فطولٌ يتراكم مقسوماً على طولٍ ينكمش**: تبلغ النسبةُ واحداً
    // فيُقصّ الخطُّ كلُّه، **ويرى السائقُ الطريقَ وقد اختفى وهو في
    // منتصفها.**
    //
    ...
    // ══════════════════════════════════════════════════════════════════
    // **ولا يُعاد الربطُ مع كلّ نبضةِ موقع**
    // ══════════════════════════════════════════════════════════════════
    //
    // **(بلاغُ المالك ٢٠٢٦-٠٩-٠٢:** «التطبيقُ لا يستجيب» — بصورةٍ من
    // شاشته. **وهي ألعنُ من انهيار**: الانهيارُ يُرى ويُبلَّغ عنه،
    // **وهذه تترك السائقَ ينظر إلى شاشةٍ ميّتةٍ وهو يقود.**)
    //
    // **وكان المفتاحُ إحداثيّةَ السائق بدقّتها الكاملة** — فيتبدّل مع
    // كلّ نبضة، **وفي السير التجريبيّ ستّين مرّةً في الثانية.**
    //
    // **و`bind` تقرأ القرص**: تسرد مجلّدات الحزم وتقرأ وصفَ كلِّ حزمة.
    // **ستّون قراءةَ قرصٍ في الثانية في الخيط الرئيسيّ** — وخمسُ ثوانٍ
    // تكفي أندرويد ليقول «لا يستجيب».
    //
    // **والإحداثيّةُ تُقرَّب إلى عُشر درجة** — نحوَ أحدَ عشرَ كيلومتراً،
    // **وهي دقّةٌ تكفي لاختيار حزمةِ منطقة** ولا تصلح لشيءٍ آخرَ هنا.
    // **فيُعاد الربطُ حين ينتقل بين المناطق لا حين يخطو مترا.**
    val bindLat = driver?.latitude?.let { kotlin.math.round(it * 10.0) / 10.0 }
    val bindLng = driver?.longitude?.let { kotlin.math.round(it * 10.0) / 10.0 }
    val binding = remember(online, routeBbox, bindLat, bindLng) {
