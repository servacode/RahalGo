...
class VoiceTest {
    ...
    /**
     * **VOICE-072** — وسقفُ التمهيد يتبع السرعة.
     *
     * **(قِيس ٢٠٢٦-٠٩-٠٢: مقطعا «كيلومتر ونصف» و«كيلومترين» مسجَّلان
     * ولا يُنطقان أبداً.)**
     */
    @Test
    fun `VOICE-072 التمهيدُ يبعد على السريع ويقرب في المدينة`() {
        val planner = VoicePlanner()
        val turn = NavManeuver(ManeuverKinds.TURN_RIGHT, "right", 100.0, 0, 50.0, 5.0, null)

        // **ثلاثون كيلومتراً في الساعة** — حيٌّ في الرقّة.
        val city = planner.triggerFor(CueStage.PREPARE, turn, 8.3)
        // **ومئةٌ وعشرون** — أوتوستراد.
        val highway = planner.triggerFor(CueStage.PREPARE, turn, 33.3)

        println("VOICE-072 · مدينة=${city.toInt()}م · سريع=${highway.toInt()}م")
        assertTrue("سقفُ المدينة ارتفع", city <= 800.0)
        assertTrue("السريعُ لم يتجاوز سقفَ المدينة", highway > 800.0)
        assertTrue("تجاوز أبعدَ مقطعٍ مسجَّل", highway <= 2000.0)

        // **والمسافةُ المنطوقةُ لها مقطعٌ مسجَّل.**
        assertTrue(planner.roundMeters(highway) in NavClips.DISTANCES)
    }
