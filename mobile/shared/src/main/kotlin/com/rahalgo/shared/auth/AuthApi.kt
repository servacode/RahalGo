...
class AuthApi(private val api: ApiClient) {
    ...
    /**
     * **جلسةُ زبونِ QA — على التجهيز وحدَه** (للاختبار الآليّ الحيّ).
     *
     * **يبطلها الخادمُ خارجَ التجهيز بـ`404`** — فلا بابَ في الإنتاج.
     * تُصدر جلسةَ زبونٍ عاديّةً بلا OTP يدويّ.
     */
    suspend fun qaSession(): AuthResult =
        api.raw(
            "/api/v1/qa/session",
            HttpMethod.Post,
