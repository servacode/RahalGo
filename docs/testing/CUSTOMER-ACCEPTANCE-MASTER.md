...
| CUST-09-026 | Market | Banner slider and banner tap (added) | Signed-in test customer · Staging · SM-A525F · valid default address · banners with targets | Observe auto-rotation; tap a banner with a target | Rotation per `banner_auto/banner_every_ms`; a clickable banner must respond | CAF-11 CONFIRMED -> CUST-DEF-008 (P3): banners with a target are clickable but ShopScreen passes no tap handler -> silent tap. No banners on staging to witness live; source-confirmed. Expected FAIL | `FAIL` | staging API | online | `/public/home` banners | — | — | — | Added. CAF-11 CONFIRMED (§40.10): banners with a target are clickable but ShopScreen passes no handler — silent tap, expected FAIL |
...
| CUST-12-021 | Checkout | Offline checkout follows blocking contract | Signed-in test customer · Staging · SM-A525F · valid default address · cart populated · offline | Open cart; tap send | OFFLINE state; send blocked | 2026-09-21 (A5): SOURCE contract now built — send button gated on central `Net.online` (`CartScreen`), so offline = disabled button + NetBanner reason (no false success / silent tap); `OfflineCheckoutBlockingTest` + negative witness. Device witness still BLOCKED (ADB net-cut drops wireless debug) | `BLOCKED` | - | offline | no order | — | — | — | §7 — source built, device witness pending |
...
**Status now (2026-09-21 — authoritative row-level count; Batch-2 in progress):** `NOT_TESTED` 292 · `NOT_APPLICABLE` 8 · `PASS` 203 · `FAIL` 1 · `BLOCKED` 74. **Open mandatory rows: 367** (NOT_TESTED 292 + BLOCKED 74 + FAIL 1). Executed groups: CUST-00…CUST-12. FAIL (1): CUST-09-026→CUST-DEF-008 (P3). Batch-2 CLOSED: CUST-03-003→CUST-DEF-006, CUST-06-032→CUST-DEF-007, CUST-06-027→CUST-DEF-010 (all PASS). Counted directly from the master's case-row status cells.
...
| | **Total** | **578** | **474** | **104** | **292** | **8** | **203** | **1** | **74** |
...
| **CUST-DEF-008** | **P3** | home/shop banner tap does nothing (was CAF-11) | **RE-AUDITED (source) 2026-09-21 — NOT a wiring bug; a product-contract decision.** The `target` field exists end-to-end (DB `banners.target`, backend `Banner.Target`, mobile `Banner.target`/`BannerSlide.target`) but **no admin surface can set it** — the banner form has no title/link field by owner decision 2026-08-09 («لا يوجد داعٍ لعنوان البانر ولا للزرّ»); default is always `""`, and no migration/seed sets one. `BannerSlider` **already** treats empty-target banners as non-actionable (no `clickable`), so today's silent tap is the **correct** contract: banners are non-actionable by owner decision. `ShopScreen` passes no `onOpen`, so a *hypothetical* non-empty target (reachable only via raw DB, not the product) would show a dead ripple — unreachable in production. **Making banners navigate is a NEW multi-surface feature** (admin target field + a target grammar + deep-link routing) that contradicts a standing owner decision → needs written analysis + owner approval per «لا تنفيذَ قبل تحليلٍ مكتوب»; **not** an authorized autonomous fix. **Action taken:** contract locked by `CustBannerTargetTest` (5 tests, negative-witnessed); no behavior change. **Recommend owner ratify CUST-09-026 FAIL → N/A** (feature intentionally absent). Left as FAIL pending ratification (578 total unchanged). **NOT a launch blocker.** |
