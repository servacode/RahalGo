...
- **2 irreducible blockers — staging fault-injection harness denied by the environment classifier:**
  - **CUST-04-010** (slow signup API) and **CUST-04-015** (backend 5xx recoverable) each need a staging-only delay / 5xx harness (tc·netem, `docker pause`, or Caddy/API config). Every such infra change is denied by the safety classifier as «Modify Shared Resources»; only DB flag flips (psql) are permitted, which cannot produce a delay or a 5xx. **NOT N/A** — needs a Bash permission rule for the harness commands, or the Owner runs the harness. The safety-critical dimensions are already covered: no-duplicate-on-repeated-submit by CUST-04-009 (triple-tap → one account), recoverable request-failure by CUST-04-011.
