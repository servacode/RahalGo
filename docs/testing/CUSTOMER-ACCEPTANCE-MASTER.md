...
| **CUST-DEF-012** | **P2** | ANR: main-thread deadlock in the map renderer on map-view detach | **CONFIRMED (device ANR trace) 2026-09-23 (OPEN)** — physical SM-A525F showed a system ANR («رحّال غو لا يستجيب») on the Customer **add-address (map) screen** during the staging outage. Dropbox `data_app_anr@…702` (2026-09-23 23:47:28): main thread (tid=1) blocked >10s in a native `condition_variable::wait` inside **`mbgl::android::MapRenderer::onRendererReset`**, via `MapLibreSurfaceView.onDetachedFromWindow` → `SurfaceViewMapRenderer.onSurfaceViewDetached` → `MapRenderer.nativeReset` — an **unbounded main-thread wait on the render thread during view detach**. `RenderThread 620` (tid=37) was parked in `MapLibreVulkanSurfaceView$VulkanThread` (MapLibre **13.4.1 Vulkan** backend), so the reset never completed → deadlock → ANR. OkHttp threads (tid=48/57, incl. the live `RealWebSocket.loopReader`) were stuck in `socketRead0` to the down `staging-api.rahalgo.com`, which left the map mid-render at detach. **Root cause is the app/library main-thread block, NOT merely staging being down** — leaving the map screen can hang the UI thread whenever the render thread is stuck. **Answers**: (1) main-thread-blocking bug = yes (`nativeReset`, no timeout); (2) requests can hang = yes (socket reads incl. WebSocket stalled on a dead backend); (3) missing timeout/cancellation = yes (map reset + socket reads). **Proposed fix (shared `map` module — pending owner approval)**: create `MapView` with TextureView/GLES rendering (`MapLibreMapOptions.textureMode(true)` / force GLES) to avoid the Vulkan `SurfaceView` detach deadlock in `map/MapHost.kt`; secondary: bound OkHttp read timeout + harden the live-WebSocket so reads to a dead backend don't hang. Affects all four apps' maps. **NOT dismissed as staging-only.** Needs build + device re-witness (open map → leave → no hang). |
