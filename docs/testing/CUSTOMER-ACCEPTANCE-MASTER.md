...
| CUST-06-032 | Auth | Session-level 403 codes are not shown as network failures (added) | Temporary password / blocked / suspended account | Launch and act | Explicit account-state message, never «لا اتصال» | FAIL (CAF-10): a 403 user_suspended at startup is shown as the offline screen (لا يوجد اتصال), not an account-state message. P2 UX defect | `FAIL` | SM-A525F/A14 vc12 | online | — | — | — | — | Added. CAF-10: 403 on `/auth/me` at startup renders the offline state |
...
| CUST-12-021 | Checkout | Offline checkout follows blocking contract | Signed-in test customer · Staging · SM-A525F · valid default address · cart populated · offline | Open cart; tap send | OFFLINE state; send blocked | Offline: ADB net-cut drops wireless debug; maps to S7 blocking gap (P8-DEF-001) | `BLOCKED` | - | offline | no order | — | — | — | §7 |
...
**Status now (2026-09-21 — authoritative row-level count; Batch-2 in progress):** `NOT_TESTED` 292 · `NOT_APPLICABLE` 8 · `PASS` 201 · `FAIL` 3 · `BLOCKED` 74. **Open mandatory rows: 369** (NOT_TESTED 292 + BLOCKED 74 + FAIL 3). Executed groups: CUST-00…CUST-12. FAILs (3): CUST-06-027→forced-password-change CONTRACT_MISMATCH (P2, no dedicated ID), CUST-06-032→CUST-DEF-007 (P2), CUST-09-026→CUST-DEF-008 (P3). CUST-03-003→CUST-DEF-006 CLOSED (Batch-2, PASS). Counted directly from the master's case-row status cells.
...
| | **Total** | **578** | **474** | **104** | **292** | **8** | **201** | **3** | **74** |
...
| **CUST-DEF-007** | **P2** | account-state 403 rendered as an offline error (was CAF-10) | **CONFIRMED (device) 2026-09-20** — a session-level 403 `user_suspended` on `/auth/me` at startup renders the OfflineScreen «لا يوجد اتصال بالإنترنت» + retry, instead of an explicit account-state (suspended) message. A suspended user is told "no internet" rather than why. Backend enforcement is correct (403, CUST-06-020); this is a client presentation gap. **NOT a launch blocker** — the user is still locked out; the message is misleading, no data/security impact. Surfaced by CUST-06-032 (also affects 021/031). Documented, not patched during acceptance |
| **CUST-DEF-008** | **P3** | home banner with a target is not clickable (was CAF-11) | **CONFIRMED (source) 2026-09-20** — banners carry a `target` and the banner component supports a tap, but `ShopScreen` passes no tap handler, so a banner tap is silent. Staging has zero banners so it can't be witnessed live; source-confirmed + audit CAF-11. **NOT a launch blocker** — cosmetic/marketing; no banners configured. Surfaced by CUST-09-026. Documented, not patched |
