...
| ID | Area | Scenario | Pre | Steps | Expected | Actual | Status | Device/Build | Net | SoT | Evidence | Defect | Regression | Notes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| CUST-04-001 | Signup | Valid Customer signup | Signed out (guest) · Staging · SM-A525F · `auth.signup_verify`=true (Staging = Production policy) | إنشاء حساب → phone → code (Staging dev OTP from staging log) → name + password + confirm (+ optional ref) → confirm | Account created; signed in; lands on تسوق | API+OTP-from-log: request→confirm(valid) → 200, account created (tokens redacted). NOTE: signup granted a 15-unit new-account gift (double-entry ledger) — cleanup pending Owner approval (see §39.0) | `PASS` | staging API | online | users +1; `auth.signup` audit | — | — | — | Fields from `ui/SignupScreen.kt`: phone, code, full name, password, confirm, referral code |
...
**Blocker-sweep run 2026-09-20 — cleared 8 of 34:** CUST-01-009, CUST-01-010 (deterministic `pm install-create`/`abandon` + size-reservation on emulator); CUST-02-010 (reversible dead-proxy → recoverable failure, not empty market); CUST-04-001/007/012/013/016 (authorized OTP-from-log signup API harness). Remaining 26 are gated by two external constraints:

- **Ledger-cleanup pending Owner decision (safety boundary):** the CUST-04-001 valid-signup granted a **15-unit "new-account gift"** as a double-entry ledger posting (`wallet_transactions` rows 22 +15 to disposable `f43d0680`, 23 −15 from pool `64155bb7`). Removing the disposable account + restoring the pool is a ledger + shared-account mutation that the safety classifier blocks and the ledger-guard reserves for explicit Owner approval. State is currently **invariant-consistent**; the disposable account remains until the Owner authorizes the cleanup (exact SQL prepared).
- **SM-A525F disconnected (UI fidelity):** the remaining UI-flow cases need the real device (wireless dropped; needs physical re-enable) — the emulator is too flaky for their multi-step navigation:
  - **CUST-02-008** signup-OFF closed **app state** (server gate already proven by CUST-04-002/023)
  - **CUST-03-003/004/005/006/007/009/010/011/012/013/014** location-UI («موقعي»), reduced-mode/notification-denied ordering, OS-location-OFF, approximate/precise location, camera/gallery avatar
  - **CUST-04-003/004/008/009/010/011/014/015/017/018/019/020/021/022** signup-UI (button-disabled, whitespace, triple-tap, slow, network-lost, 4xx-mapped-UI, 5xx-harness, back-nav, kill/reopen, verify=false, referral-UI, deep-link)
