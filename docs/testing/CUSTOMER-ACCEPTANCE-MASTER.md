...
| CUST-01-009 | Install | Interrupted/failed installation does not leave two Customer apps | Emulator | Abort an install mid-stream; list packages | Old install intact or absent; never two | not run: `adb install` is atomic — cannot deterministically interrupt mid-stream; Android's installer is transactional so a partial install cannot leave two packages. Invariant confirmed: every install/-r/-d this run yielded exactly 1 package | `BLOCKED` | emu RahalGo/A16 | any | — | (method not reproducible) | — | — | Emulator only; needs low-level interruption tooling |
| CUST-01-010 | Install | Insufficient-storage failure does not corrupt the existing install | Emulator with filled storage | Attempt update | Android error; previous install still launches | not run: filling `/data` to force `INSTALL_FAILED_INSUFFICIENT_STORAGE` risks destabilising the emulator; Android returns the error and keeps the prior install (platform guarantee) | `BLOCKED` | emu RahalGo/A16 | any | — | (deferred — env risk) | — | — | Emulator only |
...
| ID | Area | Scenario | Pre | Steps | Expected | Actual | Status | Device/Build | Net | SoT | Evidence | Defect | Regression | Notes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| CUST-04-001 | Signup | Valid Customer signup | Signed out (guest) · Staging · SM-A525F · `auth.signup_verify`=true (Staging = Production policy) | إنشاء حساب → phone → code (Staging dev OTP from staging log) → name + password + confirm (+ optional ref) → confirm | Account created; signed in; lands on تسوق | deferred to blocker-sweep — signup needs the OTP-from-staging-log harness and/or flaky signup-UI navigation (Owner directive); the server-contract gate/validation cases 002/005/006/023 are verified | `BLOCKED` | — | online | users +1; `auth.signup` audit | — | — | — | Fields from `ui/SignupScreen.kt`: phone, code, full name, password, confirm, referral code |
...
| CUST-04-007 | Signup | Canonical Syrian phone handling | Signed out (guest) · Staging · SM-A525F | Enter 09…, 9639…, +9639…, 009639… | All normalize to +9639…; same account | deferred to blocker-sweep — signup needs the OTP-from-staging-log harness and/or flaky signup-UI navigation (Owner directive); the server-contract gate/validation cases 002/005/006/023 are verified | `BLOCKED` | — | online | phone stored canonical | — | — | — | — |
| CUST-04-008 | Signup | Leading/trailing whitespace | Signed out (guest) · Staging · SM-A525F | Phone/name with spaces | Trimmed; accepted | deferred to blocker-sweep — signup needs the OTP-from-staging-log harness and/or flaky signup-UI navigation (Owner directive); the server-contract gate/validation cases 002/005/006/023 are verified | `BLOCKED` | — | online | stored trimmed | — | — | — | — |
...
| CUST-04-010 | Signup | Slow signup API | Signed out (guest) · Staging · SM-A525F | Slow network; submit | Loading then result; no duplicate | deferred to blocker-sweep — signup needs the OTP-from-staging-log harness and/or flaky signup-UI navigation (Owner directive); the server-contract gate/validation cases 002/005/006/023 are verified | `BLOCKED` | — | slow | users +1 | — | — | — | — |
| CUST-04-011 | Signup | Network lost during signup submission | Signed out (guest) · Staging · SM-A525F | Cut network after submit | Explicit failure; retry safe; no duplicate account | deferred to blocker-sweep — signup needs the OTP-from-staging-log harness and/or flaky signup-UI navigation (Owner directive); the server-contract gate/validation cases 002/005/006/023 are verified | `BLOCKED` | — | cut | users +0/+1 | — | — | — | — |
| CUST-04-012 | Signup | Server validation failure | Signed out (guest) · Staging · SM-A525F | Weak password (< `security.password_min_length`) | Explicit `weak_password` message | deferred to blocker-sweep — signup needs the OTP-from-staging-log harness and/or flaky signup-UI navigation (Owner directive); the server-contract gate/validation cases 002/005/006/023 are verified | `BLOCKED` | — | online | — | — | — | — | Client ignores `password_min_length` — server is the guard |
| CUST-04-013 | Signup | Existing phone/account | Signed out (guest) · Staging · SM-A525F | Signup with an existing phone | Explicit 'already registered' → login path | deferred to blocker-sweep — signup needs the OTP-from-staging-log harness and/or flaky signup-UI navigation (Owner directive); the server-contract gate/validation cases 002/005/006/023 are verified | `BLOCKED` | — | online | no new user | — | — | — | — |
...
| CUST-04-015 | Signup | Backend 5xx recoverable | Signed out (guest) · Staging · SM-A525F | Fault injection (harness) | Recoverable failure | deferred to blocker-sweep — signup needs the OTP-from-staging-log harness and/or flaky signup-UI navigation (Owner directive); the server-contract gate/validation cases 002/005/006/023 are verified | `BLOCKED` | — | online | — | — | — | — | Needs approved harness |
| CUST-04-016 | Signup | No duplicate accounts on repeated submission | Signed out (guest) · Staging · SM-A525F | Replay confirm | Second confirm rejected (code consumed); one account | deferred to blocker-sweep — signup needs the OTP-from-staging-log harness and/or flaky signup-UI navigation (Owner directive); the server-contract gate/validation cases 002/005/006/023 are verified | `BLOCKED` | — | online | users +1 | — | — | — | — |
| CUST-04-017 | Signup | Back navigation during registration | Signed out (guest) · Staging · SM-A525F | BACK at each step | Returns safely; no half account | deferred to blocker-sweep — signup needs the OTP-from-staging-log harness and/or flaky signup-UI navigation (Owner directive); the server-contract gate/validation cases 002/005/006/023 are verified | `BLOCKED` | — | online | users unchanged | — | — | — | — |
...
| CUST-04-019 | Signup | Signup with signup_verify=false (added) | Signed out (guest) · Staging · SM-A525F · flag false (Staging only, Owner-approved) | Phone → details directly (button «متابعة») | Account created without code; later ordering rules per contract | deferred to blocker-sweep — signup needs the OTP-from-staging-log harness and/or flaky signup-UI navigation (Owner directive); the server-contract gate/validation cases 002/005/006/023 are verified | `BLOCKED` | — | online | users +1 | — | — | — | Added: client skips the code step when false (`AuthViewModel:419-519`). Production policy is true — test only if Owner approves a temporary flip |
| CUST-04-020 | Signup | Referral code at signup (added) | Signed out (guest) · Staging · SM-A525F · valid referral code | Signup with ref (typed or pre-filled) | Referral attached; invite counts update for the referrer | deferred to blocker-sweep — signup needs the OTP-from-staging-log harness and/or flaky signup-UI navigation (Owner directive); the server-contract gate/validation cases 002/005/006/023 are verified | `BLOCKED` | — | online | `referrals` row | — | — | — | Added: ref field + invite link `https://rahalgo.com/signup?ref=` + Play install referrer |
| CUST-04-021 | Signup | Invalid referral code (added) | Signed out (guest) · Staging · SM-A525F | Signup with unknown ref | Account created; ref ignored or explicit notice; never blocks signup | deferred to blocker-sweep — signup needs the OTP-from-staging-log harness and/or flaky signup-UI navigation (Owner directive); the server-contract gate/validation cases 002/005/006/023 are verified | `BLOCKED` | — | online | no referral row | — | — | — | — |
| CUST-04-022 | Signup | Invite deep link opens signup with code (added) | Signed out (guest) · Staging · SM-A525F | `adb shell am start -d 'https://rahalgo.com/signup?ref=CODE'` | Signup opens with the code pre-filled | deferred to blocker-sweep — signup needs the OTP-from-staging-log harness and/or flaky signup-UI navigation (Owner directive); the server-contract gate/validation cases 002/005/006/023 are verified | `BLOCKED` | — | online | — | — | — | — | Only `/signup` is in the intent filter; `/i/CODE` parsed but not routed |
...
**Status now:** `NOT_TESTED` 556 · `NOT_APPLICABLE` 7 · `PASS` 0 · `FAIL` 0 · `BLOCKED` 0. **Open mandatory rows: 556.** Nothing has been executed under this document.
...
| | **Total** | **578** | **474** | **104** | **497** | **7** | **40** | **0** | **34** |
...
- **CUST-04** all signup-flow/validation-via-OTP/referral/deep-link cases except 002/005/006/023 — need OTP-from-log harness or signup-UI (sweep)
