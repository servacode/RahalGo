...
## الحكم — بعد نشر الإنتاج (٢٠٢٦-٠٩-١٨ ٢٣:١٦ UTC)

	التكافؤُ الوظيفيُّ الكامل = تحقّق — 100%
	سطوحُ الجرد               = 372  (لم يُنقَص منها شيء)
	PARITY_OK                 = 341
	ENVIRONMENT_ONLY          = 13
	DATA_ONLY                 = 18
	GENUINE_MISMATCHES        = 0
	BLOCKED_BY_PRODUCTION_VERSION = 0

**والبيئتان تشغّلان الصورتين نفسَيهما بالبصمة** — المحرّكُ
`sha256:e6bb3dfb…` والويبُ `sha256:3a3e569d…` — **والمخطّطُ `0157` فيهما.**
**وما بقي فرقاً إمّا بيئةٌ وإمّا بيانات** (§١ · §٦). التفصيلُ في §١٠.
...
| G4 | حذفُ قسمٍ مشغول: الإنتاج ٥٠٠ | ✅ أُغلق بنشر الإنتاج (§١٠) |
| G5 | إيقافُ المنصّة بلا سطرِ تدقيقٍ في الإنتاج | ✅ أُغلق بنشر الإنتاج |
| G6 | اسمُ ذلك الفعل غائبٌ عن سجلّ الإنتاج | ✅ أُغلق — النصُّ في حزمة الإنتاج (قِيس) |
| G7 | الهجرتان ٠١٥٦ و٠١٥٧ | ✅ أُغلق — `0157` · `RESTRICT` في البيئتين |

**وG4 وG5 أُثبتا بتطابق الصورة والمخطّط لا بفعلِ كتابةٍ على الإنتاج** —
**الفحصُ على الإنتاج لا يكتب** (أمرُ المالك)، **والسلوكُ مثبتٌ في التجهيز
بالصورة نفسِها وفي `admin_section_delete_test.go`.**

**وسياسةُ التجهيز سياسةُ الإنتاج**: ٢٩ إعداداً سُوّيت بباب اللوحة (§٦)،
**وبقي فرقان مقصودان** (`site.show_login` · `site.show_shop`).
...
|---|---|---|---|---|---|
| صفحاتُ اللوحة | 29 | 28 | 1 | 0 | 0 |
| أبوابُ API (86 قراءة · 89 كتابة) | 175 | 175 | 0 | 0 | 0 |
| مفاتيحُ الإعدادات | 143 | 119 | 8 | 16 | 0 |
| الأدوار | 18 | 16 | 0 | 2 | 0 |
| مفاتيحُ التهيئة | 5 | 1 | 4 | 0 | 0 |
| فهرسُ الأقسام | 1 | 1 | 0 | 0 | 0 |
| حالُ المخطّط | 1 | 1 | 0 | 0 | 0 |
| **المجموع** | **372** | **341** | **13** | **18** | **0** |

	التدقيقُ الأوّل            304 · 10 · 18 · 40 فيها فرق
	بعد قرارات المالك          336 · 13 · 18 · 0 · BLOCKED 5
	بعد نشر الإنتاج            341 · 13 · 18 · 0 · BLOCKED 0

**وما بقي في ENV**: صفحةُ `/dashboard/ops` (اسمُ البيئة) · أعلامُ الإطلاق
الستّة وB الاثنان (الإنتاجُ قبلَ الافتتاح) · العنوانُ والموقعُ واسمُ البيئة
ومضيفُ الخرائط. **وفي DATA**: محتوى الهويّة والصفحات (16) · ودورا اختبارٍ
في التجهيز (2).
...
|---|---|---|
| ويبُ اللوحة | ~~`release-79d88245`~~ ⇐ **`release-68a45c97`** (`3a3e569d…`) | `release-68a45c97` (`3a3e569d…`) |
| المحرّك | ~~`release-79d88245` · `0155`~~ ⇐ **`release-68a45c97` · `0157`** (`e6bb3dfb…`) | `release-68a45c97` · `0157` (`e6bb3dfb…`) |
...
**البصمةُ واحدة** (الاسمُ والأيقونةُ والترتيب). **والعقدُ واحدٌ بعد النشر**:
حذفُ قسمٍ مشغول ٤٠٩ في البيئتين، والمفتاحُ `RESTRICT` فيهما (G4 · G7).
...
| `/dashboard/sections` | content.manage | 200 | 200 | PARITY_OK (G4 أُغلق بالنشر) |
...
| `/dashboard/audit` | audit.read | 200 | 200 | PARITY_OK (G6 أُغلق بالنشر) |
...
# ٨ · حالُ التجهيز بعد القرارات
	...
	الإنتاج      = لم يُمَسّ حتّى النشر المأذون (§١٠)

---

# ٩ · الفحصُ المسبق لنشر الإنتاج — **كُتب قبل الإذن** (نُفّذ في §١٠)
...
# ١٠ · نشرُ الإنتاج — نُفّذ بإذن المالك (٢٠٢٦-٠٩-١٨ UTC)

> **إذنُ المالك**: «OWNER AUTHORIZATION = APPROVED» — الهدفُ `68a45c97`
> بالصورتين المُثبتتين في التجهيز · **بلا بناء** · الهجرتان ٠١٥٦ و٠١٥٧ وحدَهما.

## ١٠·١ · النسخُ الاحتياطيّ — قبل أيّ تبديل

	pg_dump   /srv/rahalgo/backups/rahalgo-pre68a45c97-20260918T230705Z.dump
	          2607792 bytes · 94 جدولَ بيانات في فهرسه (قُرئ بـpg_restore -l)
	          sha256 a12228cee0e01cb2e29deee25c24c9892265de4e6471385bae288ee275b44728
	المحرّك   /srv/rahalgo/artifacts/rahalgo-api-release-79d88245.tar
	          index sha256:187053ae… = الصورةُ العاملة · sha256 042a12004eb066564b528455433469e8fe7be8a7aacd85d28cf7bfa4c33621de
	الويب     /srv/rahalgo/artifacts/rahalgo-web-release-79d88245.tar
	          index sha256:ddebbc60… = الصورةُ العاملة · sha256 c0118d7268c208394e98e4a91334b245ffa0386d832ee3392fe37764ef510041
	Caddy     /srv/rahalgo/backups/Caddyfile.pre68a45c97-20260918T230705Z
	          sha256 084d37292cc2a64bf219447bbde56e9c9bc87bb3e9d5380723b119d6cc7f143d (فيه كتلةُ staging-api)
	.env      /srv/rahalgo/deploy/.env.bak-pre-68a45c97

> **تصحيح:** **فحصي الأوّلُ للأرشيف قارن بصمةَ الإعداد (`config`) ببصمة
> الصورة فسقط** — **ومخزنُ الصور هنا `containerd`، وبصمةُ الصورة فيه بصمةُ
> الفهرس (`index.json`) لا الإعداد.** **فوقفتُ قبل أيّ نشر، وحذفتُ الأرشيفَ
> الذي أنشأتُه قبل دقيقة، وأعدتُ النسخَ كلَّه بفحصٍ صحيح** (بصمةُ الفهرس =
> الصورةُ العاملة، وبصمةُ محتواه محسوبة). **وبقيت نسخةُ القاعدة الأولى**
> (`…T230554Z.dump`) سليمةً إلى جانب الثانية.

## ١٠·٢ · النشر

	المحرّك  23:09:10–23:09:12  promote.sh · TARGET_ENV=production
	         الصورة sha256:e6bb3dfb… · source 68a45c97 · migrations applied count=2
	الويب    23:16:11–23:16:17  up -d --no-build --no-deps web
	         الصورة sha256:3a3e569d… · /adminrahalgo 200 بعد 4 ثوانٍ
	.env     سطرا الصورتين وحدَهما ⇐ 68a45c97 · وبقيّةُ الأسطر مطابقةٌ للنسخة حرفاً

**والويبُ فُحص قبل النشر**: **صفرُ ملفٍّ فيه `staging-api.rahalgo.com`** —
**كصورة الإنتاج السابقة تماماً** (والملامحُ الأربعةُ متطابقة) — **فالتهيئةُ
وقتَ التشغيل وحدَها.**

## ١٠·٣ · قبل وبعد — ٨٨ جدولاً ببصمة محتواها

	جداولُ متطابقةٌ حرفاً      87 / 88   (users · orders · wallets · wallet_transactions ·
	                                       app_settings · audit_log · merchants · menu_items …)
	تبدّل                     schema_migrations 155 ⇐ 157 (وحدَه)
	بصمةُ الإعدادات           15a05ee79d7a5eaa5b222cc2aedcc1d6 = قبل وبعد
	بصمةُ الصلاحيّات          f37db9c441e6535ef23f71b44bcd4033 (107) = قبل وبعد
	أدوارُ المستخدمين          e9de388a7b28f546ac55a05b55b1b2cd (4) = قبل وبعد
	أعلامُ الإطلاق            false كلُّها = قبل وبعد
	Caddyfile · compose.yml   بصمتاهما = قبل وبعد
	moneycheck               51 · 0 خرق — قبل وبعد · **والمخرَجُ متطابقٌ سطراً سطراً**
	                         (اتّصالٌ للقراءة فقط: default_transaction_read_only=on · والكتابةُ رُفضت)

## ١٠·٤ · الهجرتان

	0156_launch_sections.sql          applied 23:09:12.225
	0157_section_delete_contract.sql  applied 23:09:12.240
	الأقسامُ الفعّالة       38 / 38 · b4a77017bb32e32812935ec26402f24d (كما كانت)
	platform_sections_active_name_uq  موجود
	menu_items → platform_sections    ON DELETE RESTRICT
	menu_items                         0 · بلا قسم 0 · يتيمة 0

## ١٠·٥ · الفحوصُ الثلاثةَ عشر

| # | الفحص | النتيجة |
|---|---|---|
| 1 | الهويّة | ✅ production · `68a45c97` · `0157` |
| 2 | سجلُّ المحرّك | ✅ `migrations applied count=2` · لا خطأ · 0 إعادة تشغيل |
| 3 | الأقسام | ✅ 38 · `b4a77017…` |
| 4 | الفهرسُ والمفتاح | ✅ موجود · `r` |
| 5 | بصمةُ الإعدادات | ✅ لم تتبدّل |
| 6 | moneycheck | ✅ 51/51 |
| 7 | أبوابُ القراءة (86) | ✅ 401 كلُّها — في البيئتين |
| 8 | صفحاتُ اللوحة (29) | ✅ 200 كلُّها — في البيئتين |
| 9 | `config.js` | ✅ api.rahalgo.com · production · maps.rahalgo.com |
| 10 | خرائطُ الإنتاج | ✅ النمط 200 · البلاطة 206 |
| 11 | التجهيز | ✅ staging · `68a45c97` · `0157` · وخرائطُه 200 |
| 12 | `/api/v1/public/sections` | ✅ 200 · فيه `orderable_now` |
| 13 | اللوحة | ✅ **بديلٌ آليّ**: النصّان الجديدان في حزمة لوحة الإنتاج (16 قطعة) — **ولم أدخل لوحةَ الإنتاج: لا اعتمادَ مأذونٌ لي فيها** |

## ١٠·٦ · التراجع — جاهزٌ ولم يُستعمل

	docker load -i /srv/rahalgo/artifacts/rahalgo-{api,web}-release-79d88245.tar
	promote.sh ... rahalgo-api:release-79d88245 · ثمّ الويب · ثمّ .env من .env.bak-pre-68a45c97
	والقاعدةُ تبقى على 0157 — المحرّكُ القديمُ يُقلع عليها (§٩·٤)

## ١٠·٧ · متابعةٌ منفصلة — ليست من هذا النشر

**`Caddyfile` الإنتاج على الخادم فيه كتلةُ `staging-api.rahalgo.com`،
ونسخةُ المستودع (`deploy/Caddyfile`) ليست فيها.** **لم يُمَسّ في هذا النشر**
(بصمتُه قبل وبعد واحدة). **وإصلاحُ الفرق — إدخالُ الكتلة المستودعَ —
مهمّةُ تهيئةٍ مستقلّةٌ تنتظر المالك**، كي لا يقطع أحدٌ التجهيزَ بنسخ
المستودع فوق الخادم.
